...
class BlockRope(nn.Module):
    def __init__(
        ...
        self.attn = attn_class(
            ...
            proj_drop=drop,
            qk_norm=qk_norm,
            qk_norm_chunk_size=qk_norm_chunk_size,
            rope=rope
        )
...
class DSABlockRope(nn.Module):
    def __init__(
        ...
        if DSAAttention is None:
            raise ImportError(
                "DSAAttention is unavailable. Ensure easyvolcap is importable before enabling Pi3 indexer."
            )
        ...
        self.sample_drop_ratio = drop_path
        self.indexer_state = {}
        self.return_indexer_loss = False

    def forward(self, x: Tensor, xpos=None):
        indexer_loss = None

        def attn_residual_func(x: Tensor) -> Tensor:
            nonlocal indexer_loss
            self.attn.indexer_state = self.indexer_state
            attn_out, indexer_loss = self.attn(self.norm1(x), pos=xpos)
        ...
        if self.return_indexer_loss:
            if indexer_loss is None:
                state = self.indexer_state if isinstance(self.indexer_state, dict) else {}
                if bool(state.get("compute_loss", False)):
                    layer_idx = state.get("layer_idx", None)
                    raise RuntimeError(
                        "Pi3 DSA block did not produce indexer_loss while "
                        f"compute_loss=True (layer_idx={layer_idx})."
                    )
                indexer_loss = torch.zeros((), device=x.device, dtype=x.dtype)
