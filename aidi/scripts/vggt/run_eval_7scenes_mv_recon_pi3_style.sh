...
SEVEN_SCENES_ROOT_PRIMARY="/horizon-bucket/saturn_v_4dlabel/004_vision/01_users/zinan.lv/dataset_point/7Scenes"
SEVEN_SCENES_ROOT_SECONDARY="/horizon-bucket/saturn_v_4dlabel/004_vision/01_users/tao02.xie/datasets/7scenes/test"

if [ -z "${DATASET_ROOT}" ]; then
  DATASET_ROOT="$(
    first_existing_dir \
      "${SEVEN_SCENES_ROOT_PRIMARY}" \
      "${SEVEN_SCENES_ROOT_SECONDARY}" \
      "${REPO_ROOT}/tmp/7scenes_test" \
      "${REPO_PARENT}/7scenes_test" || true
  )"
fi
...
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/tmp/7scenes_mv_recon_pi3_style_${MODEL_FAMILY}_${MODEL_TAG_EFFECTIVE}_$(date +%Y%m%d_%H%M%S)}"
...
if [ -z "${DATASET_ROOT}" ] || [ ! -d "${DATASET_ROOT}" ]; then
  echo "[7scenes-mv-recon-pi3-style] ERROR: dataset root not found. Set DATASET_ROOT."
  echo "  Tried: ${SEVEN_SCENES_ROOT_PRIMARY}, ${SEVEN_SCENES_ROOT_SECONDARY}"
  exit 1
fi
...
if [ -z "${SEQ_MAP}" ]; then
  ...
  if [ "${PROTOCOL}" = "sparse" ] || [ "${PROTOCOL}" = "auto" ] || [ "${PROTOCOL}" = "both" ]; then
    sparse_local="${LOCAL_SEQ_MAP_DIR}/7scenes_mv-recon_seq-id-map-kf200.json"
    sparse_pi3="${PI3_SEQ_MAP_DIR}/7scenes_mv-recon_seq-id-map-kf200.json"
    if ! json_has_keys "${sparse_local}" && ! json_has_keys "${sparse_pi3}"; then
      need_gen=true
    fi
  fi
  if [ "${PROTOCOL}" = "dense" ] || [ "${PROTOCOL}" = "auto" ] || [ "${PROTOCOL}" = "both" ]; then
    dense_local="${LOCAL_SEQ_MAP_DIR}/7scenes_mv-recon_seq-id-map-kf40.json"
    dense_pi3="${PI3_SEQ_MAP_DIR}/7scenes_mv-recon_seq-id-map-kf40.json"
  ...
  if [ "${need_gen}" = true ]; then
    ...
    python3 aidi/scripts/baselines/generate_seq_id_map.py \
      ...
      --kf-step 200 \
      --max-views 10 \
      --output "${LOCAL_SEQ_MAP_DIR}/7scenes_mv-recon_seq-id-map-kf200.json"
    python3 aidi/scripts/baselines/generate_seq_id_map.py \
      --dataset-root "${DATASET_ROOT}" \
      --dataset 7scenes \
      --kf-step 40 \
      --max-views 10 \
      --output "${LOCAL_SEQ_MAP_DIR}/7scenes_mv-recon_seq-id-map-kf40.json"
  fi
fi
