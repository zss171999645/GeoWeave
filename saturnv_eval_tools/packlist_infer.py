...
from utils.high_freq_pose import read_ins_data, generate_high_freq_pose
...
class ClipInfer:
    def __init__(self, 
        ...
        self.weight_path_lc = weight_path_lc
        self.config_path_lc = config_path_lc
        ...
        self.add_loop_closure = add_loop_closure
    ...
    def load_model(self):
        ...
        if not self.use_cam_emb:
            pt_dict = {k:v for k, v in pt_dict.items() if "cam_mlp" not in k}
    ...
    def load_model_lc(self):
        ...
        model_cfg = {}
        if self.config_path_lc is not None:
            with open(self.config_path_lc, "r") as f:
                config = yaml.load(f, Loader=yaml.FullLoader)
            model_cfg = config["model_cfg"]["sampler_cfg"]
            del model_cfg["use_cam_emb"]
            del model_cfg["dinov2_ckpt"]
        ...
        self.model_lc = VGGTSampler(dinov2_ckpt=None, use_cam_emb=self.use_cam_emb, network=None, low_vram=self.low_vram, **model_cfg)

        print("loading model from ", self.weight_path_lc)
        pt_dict = torch.load(self.weight_path_lc, map_location=torch.device('cpu'))
        pt_dict = pt_dict["model"]
        pt_dict = {k.replace('sampler.', ''): v for k, v in pt_dict.items()}
        if not self.use_cam_emb:
            pt_dict = {k:v for k, v in pt_dict.items() if "cam_mlp" not in k}
        self.model_lc.load_state_dict(pt_dict)
        self.model_lc.eval()
        
        device = "cuda" if torch.cuda.is_available() else "cpu"
        logger.info(f"Using device: {device}, load model from {self.weight_path_lc}")
        self.model_lc = self.model_lc.to(device)
        self.model_lc.eval()
    ...
    def get_pandar_pose(self):
        ...
        # GPS mode
        pandarpose_file = join(self.pandar_site_path, "4DLabel_output/LidarSLAM/odometry", self.clip_names[0], "lio_offline_100HZ_by_odom.txt")
        if exists(pandarpose_file): 
            ...
            pandarpose_file_10Hz = join(self.pandar_site_path, "4DLabel_output/LidarSLAM_3DModel_0/odometry", "lio_offline_10HZ.txt")

            if exists(pandarpose_file): 
                pandar_chassis2w = np.loadtxt(pandarpose_file)
            elif exists(pandarpose_file_10Hz):
                liosam_10hz_pose = np.loadtxt(pandarpose_file_10Hz)
                odo_poses = []
                for clip_name in self.clip_names:
                    clip_dir = join(self.pandar_site_path, "Raw_data", clip_name)
                    odo_path = join(clip_dir, "3ddr/data.json")
                    odo_poses.append(read_ins_data(odo_path))
                # Combine all ins_frames into a single array
                odo_poses = np.vstack(odo_poses)
                sorted_indices = np.argsort(odo_poses[:, 0])
                sorted_odo_poses = odo_poses[sorted_indices]
                # Remove duplicate timestamps while preserving order (optional) # noqa: E501
                _, unique_indices = np.unique(sorted_odo_poses[:, 0],
                                                return_index=True)
                unique_sorted_odo_poses = sorted_odo_poses[unique_indices]
                lidar_timestamps = unique_sorted_odo_poses[:, 0]
                mask = ((lidar_timestamps > liosam_10hz_pose[0, 0]) &
                        (lidar_timestamps < liosam_10hz_pose[-1, 0]))
                lidar_timestamps = lidar_timestamps[mask]
                lidar_T_vcs = np.eye(4)
                lidar_poses_tum = generate_high_freq_pose(
                    lidar_timestamps,
                    unique_sorted_odo_poses,
                    liosam_10hz_pose,
                    lidar_T_vcs,
                )
                np.savetxt(
    ...
    def get_clip_pose(self):
        ...
        if exists(visual_3d_model_jpath):
            ...
            for clip_name in self.clip_names:
                ...
                for camera_name in self.camera_names:
                    clip_pose[camera_name] = {}
                    if clip_name not in clip2model:
                        continue
                    modelids = clip2model[clip_name]
                    sfm_cam2w = []
                    for modelid in modelids:
                        sfmpose_file = join(self.site_output_path, f"VisualSFM_{modelid}", "odometry_by_odom_100Hz", clip_name, f"{camera_name}.txt")
                        sfm_cam2w.extend(np.loadtxt(sfmpose_file).tolist())
                    sfm_cam2w = np.array(sorted(sfm_cam2w, key=lambda x: x[0]))
                    clip_pose[camera_name]["sfm_cam2w"] = sfm_cam2w

                    odopose_file = join(self.site_debug_path, "VisualSFM", "Vision_Result", clip_name, camera_name, f"{camera_name}_wigo_offset.txt")
                    odo_cam2w = np.loadtxt(odopose_file)
                    odo_cam2w = np.array(sorted(odo_cam2w, key=lambda x: x[0]))
                    clip_pose[camera_name]["odo_cam2w"] = odo_cam2w
                    ...
                    if self.gt_type == "pandar":
                        ...
                        # 转到各个相机坐标系
                        clip_dir = join(self.site_rawdata_path, clip_name)
                        ...
                        clip_pose[camera_name]["pandar_cam2w"] = pt.dumparray()
                        clip_pose[camera_name]["pandar_cam2w"] = np.array(sorted(clip_pose[camera_name]["pandar_cam2w"], key=lambda x: x[0]))
                    elif self.gt_type == "sfm":
                        logger.info("use sfm pose as gt")
                        clip_pose[camera_name]["pandar_cam2w"] = sfm_cam2w
                    else:
                        logger.info(f"not support gt type {self.gt_type}")
                clips_pose[clip_name] = clip_pose
        else:
            ...
            for clip_name in self.clip_names:
                ...
                for camera_name in self.camera_names:
                    ...
                    odo_cam2w = np.array(sorted(odo_cam2w, key=lambda x: x[0]))
                    clip_pose[camera_name]["odo_cam2w"] = odo_cam2w
                    clip_pose[camera_name]["sfm_cam2w"] = odo_cam2w
                    clip_pose[camera_name]["pandar_cam2w"] = odo_cam2w
                clips_pose[clip_name] = clip_pose
        return clips_pose
    ...
    def prepare_clipdata(self):
        ...
        for clip_name in self.clip_names:
            if clip_name not in clip_pose:
                continue
            clip_infos = {}
            for camera_name in self.camera_names:
                camera_info = {}
                image_dir = join(self.site_debug_path, "VisualSFM", "Vision_Result", clip_name, camera_name, "keyframe")
                if not os.path.exists(image_dir):
                    continue
                ims = sorted(os.listdir(image_dir))
                ims = [join(image_dir, im) for im in ims]
                camera_info["ims"] = ims
                camera_info["sfm_cam2w"] = clip_pose[clip_name][camera_name]["sfm_cam2w"]
                camera_info["odo_cam2w"] = clip_pose[clip_name][camera_name]["odo_cam2w"]
                camera_info["pandar_cam2w"] = clip_pose[clip_name][camera_name]["pandar_cam2w"]
                camera_param_path = join(self.site_debug_path, "VisualSFM", "Vision_Result", clip_name, camera_name, f"{camera_name}.params")
                clip_attribute_path = join(self.site_rawdata_path, clip_name, "attribute.json")
            ...
            clips_infos[clip_name] = clip_infos
            
            
        clip_infos = {}
        #merge clip_infos
        for clip_name, one_clip_infos in clips_infos.items():
            if clip_infos == {}:
                clip_infos = one_clip_infos
                continue
            for camera_name, camera_info in one_clip_infos.items():
                for k, v in camera_info.items():
                    if k == "K" or k == "pandar_cam2w":
                        continue
                    elif type(v) == list:
                        clip_infos[camera_name][k].extend(v)
                    elif type(v) == np.ndarray:
                        clip_infos[camera_name][k] = np.concatenate((clip_infos[camera_name][k], v), axis=0)
        return clip_infos
    ...
    def prepare_segment_mvseq(self, clip_infos):
        ...
        for cammera_idx, (camera_name, camera_info) in enumerate(clip_infos.items()):
            ...
            sfm_cam2w = camera_info["sfm_cam2w"]
            odo_cam2w = camera_info["odo_cam2w"]
            imgs_ts = [int(im)/1000.0 for im in sync_imgs_ts_list[cammera_idx]]

            odo_cam2w_sync = interpolate(imgs_ts, odo_cam2w)
            pandar_cam2w = np.array(sorted(camera_info["pandar_cam2w"], key=lambda x: x[0]))
            pt_pandar = PoseTransformer()
            pt_pandar.loadarray(pandar_cam2w)
            pt_odo = PoseTransformer()
            pt_odo.loadarray(odo_cam2w)
            pt_sfm = PoseTransformer()
            pt_sfm.loadarray(sfm_cam2w)
            
            sync_clip_infos[camera_name] = {"ims": sync_ims, 
                                            "Ks": Ks, 
                                            "sfm_cam2w": sfm_cam2w, 
                                            "odo_cam2w": odo_cam2w, 
                                            "odo_cam2w_sync": odo_cam2w_sync,
        ...
        if self.use_window:
            ...
            segment_idx = []
            start_idx = 0
            ref_poses = sync_clip_infos[self.camera_names[0]]["odo_cam2w_sync"]
                      
            # 定义位姿比较函数
            def check_pose_condition(idx1, idx2):
                """检查两个位姿是否满足距离或角度条件"""
                odo_cam2w1 = ref_poses[idx1]
                odo_cam2w2 = ref_poses[idx2]
                
                # 构建变换矩阵
                c2w1 = np.eye(4)
                c2w1[:3, :3] = Rotation.from_quat(odo_cam2w1[4:8]).as_matrix()
                c2w1[:3, 3] = odo_cam2w1[1:4]
                
                c2w2 = np.eye(4)
                c2w2[:3, :3] = Rotation.from_quat(odo_cam2w2[4:8]).as_matrix()
                c2w2[:3, 3] = odo_cam2w2[1:4]
                
                # 计算相机到世界变换的逆（世界到相机）
                w2c1 = np.linalg.inv(c2w1)
                w2c2 = np.linalg.inv(c2w2)
                
                # 计算平移距离
                dist = np.linalg.norm(w2c1[:3, 3] - w2c2[:3, 3])
                ...
                return dist > 1.5 or angle_deg > 1.5
            # 尝试为每个起始位置构建batch
            for start_idx in range(0, length, self.overlap_size):
                current_batch = [start_idx]
                last_added_idx = start_idx
                
                # 尝试向后扩展
                next_idx = start_idx + 1
                while len(current_batch) < self.window_size and next_idx < length:
                    if check_pose_condition(last_added_idx, next_idx):
                        current_batch.append(next_idx)
                        last_added_idx = next_idx
                    next_idx += 1
                
                # 如果向后扩展不足，尝试向前扩展
                if len(current_batch) < self.window_size:
                    prev_idx = start_idx - 1
                    while prev_idx >= 0 and len(current_batch) < self.window_size:
                        if check_pose_condition(last_added_idx, prev_idx):
                            current_batch.append(prev_idx)  # 添加到开头
                            last_added_idx = prev_idx  # 更新最后添加的索引
                        prev_idx -= 1
                
                # 如果成功收集到完整batch，保存结果
                if len(current_batch) == self.window_size:
                    segment_idx.append(current_batch)
                else:
                    logger.info(f"Failed to find a complete batch for {start_idx}")
                    exit(-1)
        # ==================== 回环检测与segment构建 ====================
        def detect_loop_closures(ref_poses, distance_threshold=5.0, min_time_gap=50):
            """
            检测回环候选帧
            Args:
                ref_poses: 参考相机位姿列表
                distance_threshold: 距离阈值(米)
                min_time_gap: 最小时间间隔(帧数)，避免检测相邻帧
            Returns:
                loop_pairs: 回环对列表，每个元素为(current_idx, loop_idx)
            """
            loop_pairs = []
            n_frames = len(ref_poses)
            
            # 提取所有帧的平移向量
            positions = []
            for i, pose in enumerate(ref_poses):
                c2w = np.eye(4)
                c2w[:3, :3] = Rotation.from_quat(pose[4:8]).as_matrix()
                c2w[:3, 3] = pose[1:4]
                w2c = np.linalg.inv(c2w)
                positions.append(c2w[:3, 3]) # consider x and y
            ...
            # 为每个帧寻找邻近帧
            for i in range(min_time_gap, n_frames):
                # 查询距离阈值内的所有邻近帧
                neighbors = kdtree.query_ball_point(positions[i], distance_threshold)
                ...
                for j in neighbors:
                    ...
                    if j < i - min_time_gap:
                        ...
                        if dist < distance_threshold:
                            ...
                            logger.info(f"检测到回环: 帧{i} <-> 帧{j}, 距离: {dist:.2f}m")
            
            return loop_pairs
        
        def find_segment_containing_frame(target_frame, segments):
            """找到包含目标帧的segment"""
            for seg_idx, segment in enumerate(segments):
                if target_frame in segment:
                    return seg_idx, segment
            return None, None
        
        def create_loop_segment(current_segment, loop_segment, current_frame, loop_frame, window_size=10, max_img_size=2048):
            """
            创建回环segment，合并当前segment和回环segment的帧
            """
            # 确保两个segment都包含回环帧
            if current_frame not in current_segment or loop_frame not in loop_segment:
                return None
            
            # 合并两个segment的帧，优先选择靠近回环帧的部分
            combined_frames = set()
            
            
            # 从当前segment中取window_size//2帧，以current_frame为中心
            current_start = max(0, current_frame - window_size//4)
            current_end = min(max_img_size, current_frame + window_size//4 + 1)
            current_frames = [i for i in range(current_start, current_end)]
            
            # 从回环segment中取window_size//2帧，以loop_frame为中心
            loop_start = max(0, loop_frame - window_size//4)
            loop_end = min(max_img_size, loop_frame + window_size//4 + 1)
            loop_frames = [i for i in range(loop_start, loop_end)]
            
            # 合并帧，确保总数不超过window_size
            combined_frames = set(current_frames) | set(loop_frames)
            combined_frames = sorted(combined_frames)
            
            # 如果帧数过多，选择最接近回环帧的window_size帧
            if len(combined_frames) > window_size:
                # 计算每个帧到两个回环帧的平均距离
                def frame_distance(frame):
                    dist_to_current = abs(frame - current_frame)
                    dist_to_loop = abs(frame - loop_frame)
                    return (dist_to_current + dist_to_loop) / 2
                
                combined_frames = sorted(combined_frames, key=frame_distance)[:window_size]
            
            return sorted(combined_frames)
        
        if self.add_loop_closure:
            # 检测回环
            ref_poses = sync_clip_infos[self.camera_names[0]]["odo_cam2w_sync"]
            loop_pairs = detect_loop_closures(ref_poses, distance_threshold=3.0, min_time_gap=50) # consider 5.0
            
            # 为每个回环对创建segment
            loop_segments_added = 0
            for current_frame, loop_frame in loop_pairs:
                # 找到包含当前帧和回环帧的segment
                current_seg_idx, current_segment = find_segment_containing_frame(current_frame, segment_idx)
                loop_seg_idx, loop_segment = find_segment_containing_frame(loop_frame, segment_idx)
                
                if current_segment is not None and loop_segment is not None:
                    # 创建回环segment
                    loop_segment_frames = create_loop_segment(
                        current_segment, loop_segment, current_frame, loop_frame, self.window_size, len(ref_poses)
                    )
                    
                    if loop_segment_frames and len(loop_segment_frames) >= 5:  # 至少5帧
                        # 检查是否已存在相似的segment
                        is_duplicate = False
                        for existing_segment in segment_idx:
                            if len(set(existing_segment).intersection(set(loop_segment_frames))) >= self.window_size/2+1:
                                is_duplicate = True
                                break
                        
                        if not is_duplicate:
                            segment_idx.append(loop_segment_frames)
                            loop_segments_added += 1
                            logger.info(f"添加回环segment: {loop_segment_frames}")
            
            logger.info(f"共检测到{len(loop_pairs)}个回环对，添加了{loop_segments_added}个回环segment")
            
                    
                    
        # split according to segment idx
        for segment_i in tqdm.tqdm(range(len(segment_idx))): # for each segment_idx
            batch_idx = segment_idx[segment_i]
            ...
            for camera_name, camera_info in sync_clip_infos.items():
                ...
                segment_sfm_cam2w = interpolate(imgs_ts, sfm_cam2w, pt=camera_info["pt_sfm"])
                segment_odo_cam2w = interpolate(imgs_ts, odo_cam2w, pt=camera_info["pt_odo"])
                segment_pandar_cam2w = interpolate(imgs_ts, pandar_cam2w, pt=camera_info["pt_pandar"])
                ...
                batch_save_path += [join(self.save_root, camera_name, f"{segment_i}")] * len(imgs)
            batch_sfm_cam2w = np.concatenate(batch_sfm_cam2w, axis=0)
            batch_odo_cam2w = np.concatenate(batch_odo_cam2w, axis=0)
            batch_pandar_cam2w = np.concatenate(batch_pandar_cam2w, axis=0)
            batch_segment_pose = {}
            ...
            frame_ids_sorted = sorted(batch_idx)
            is_loop_seg = not all(frame_ids_sorted[i+1] - frame_ids_sorted[i] == 1 
                           for i in range(len(frame_ids_sorted)-1))

            self.segments[segment_i].append(
                {"camera_names": batch_camera_names,
                 ...
                 "is_loop_seg": is_loop_seg})
        return sync_clip_infos
    ...
    def segment_infer(self, segment):
        ...
        Ks = segment["Ks"]
        is_loop_seg = segment["is_loop_seg"]
        logger.info(f"loading images, {len(imgs)}")
        # rgb, ixts = load_and_preprocess_images_K(imgs, Ks)
        image_load_config = self.config.model_config['dataloader_cfg']['dataset_cfg']['metaset_cfgs'][-1]
        rgb, ixts = load_and_preprocess_images_K_parallel(imgs, Ks, 
                    align_size=image_load_config['align_size'], 
                    proc_align_size=image_load_config['proc_align_size'],
                    max_size=image_load_config['max_size'])
    ...
    def save_pose(self, segment):
        # save_pose
        sfm_cam2w = sorted(segment["pose"]["sfm_cam2w"], key=lambda x: x[0])
        vggt_cam2w = sorted(segment["pose"]["vggt_cam2w"], key=lambda x: x[0])
        odo_cam2w = sorted(segment["pose"]["odo_cam2w"], key=lambda x: x[0])
        pandar_cam2w = sorted(segment["pose"]["pandar_cam2w"], key=lambda x: x[0])
        sfm_w2c = transform_pose(sfm_cam2w, type="tum") # (S, 4, 4)
        vggt_w2c = transform_pose(vggt_cam2w, type="tum") # (S, 4, 4)
        odo_w2c = transform_pose(odo_cam2w, type="tum") # (S, 4, 4)
        pandar_w2c = transform_pose(pandar_cam2w, type="tum") # (S, 4, 4)
        ...
        np.savetxt(join(segment["save_path"], "idxes.txt"), np.sort(np.array(segment["idxes"]).astype(np.int32)), fmt='%d')
        np.savetxt(join(segment["save_path"], "ims.txt"), np.sort(np.array(segment["ims"])), fmt='%s')
    ...
    def pose_eval(self, camera_name, segment):
        ...
        for p1, p2 in evo_list:
            ...
            segment_i = segment["seg_id"]
            idxes = segment["idxes"]
            logger.info(f"[{camera_name}] {segment_i} {p1}-{p2} rpe_trans: {rpe_trans:.4f}, rpe_rot: {rpe_rot:.4f}, ate_trans: {ate_trans:.4f}, ate_rot: {ate_rot:.4f}")
        ...
        for k, v in segment.items():
            if k == "pose":
                ...
                for k2, v2 in v.items():
                    segment_split[k][k2] = v2[sdix:edix]
            elif k == "save_path":
                segment_split[k] = v[sdix]
            elif (k == "seg_id") or (k == "idxes") or (k == "is_loop_seg"):
                segment_split[k] = v
            else:
    ...
    def clip_infer(self):
        ...
        if not self.mvseq:
            ...
            sync_clip_infos = self.prepare_segment_mvseq(clip_infos)
            self.load_model()
            if self.add_loop_closure:
                self.load_model_lc()
            segments_dict = {}
            ...
            for i, (segment_name, segment) in enumerate(self.segments.items()):
                ...
                logger.info(f"start infer {i}, {segment['idxes']}")
                segment, res = self.segment_infer(segment)
                logger.info(f"finish infer {i}")
                # self.save_sync_pointcloud(clip_infos, segment, res)
                # need split by camera
                logger.info(f"start save pose {i}")
                ...
                for camera_idx, camera_name in enumerate(self.camera_names):
                    ...
                    logger.info(f"finish {camera_name} {i}")
                    segments_dict[camera_name].append(segment_camera)
                logger.info(f"finish save pose {i}")
                logger.info(f"start fusion {i} out of {len(self.segments)}")
                if self.save_pointcloud:
                    self.segment_fusion(segment, res)
                logger.info(f"finish fusion {i}")
            ...
            extrinsics_camerafront2cam = []
            for camera_name in self.camera_names:
                clip_dir = join(self.site_rawdata_path, self.clip_names[0])
                dr = DatasetReader(clip_dir)
                camerafront2camera = dr.get_extrinsic("camera_front", camera_name, jump_sensor="chassis")
                tvec = camerafront2camera[:3, 3]
                qvec = Rotation.from_matrix(camerafront2camera[:3, :3]).as_quat()
                extrinsics_camerafront2cam.append([camera_name, tvec[0], tvec[1], tvec[2], qvec[0], qvec[1], qvec[2], qvec[3]])
            logger.info("save extrinsics.txt")
            np.savetxt(join(self.save_root, f"extrinsics.txt"), np.array(extrinsics_camerafront2cam, dtype=object), fmt='%s')
            ...
            for camera_name in ["camera_front"]:
                np.savetxt(join(self.save_root, "before_optimization", f"{camera_name}_list.txt"), sync_clip_infos[camera_name]['ims'], fmt='%s')

                trajres, debug_pose = cam_pose_eval(clip_infos, camera_name, cam_posefile, "_pgo")
                pose_save_path = join(self.save_root, f"pose_eval_7dof")
                if not exists(pose_save_path):
                    os.makedirs(pose_save_path)
                for posetype, pose in debug_pose.items():
                    np.savetxt(join(pose_save_path, f"{camera_name}-{posetype}.txt"), pose)
                with open(join(pose_save_path, f"{camera_name}.json"), "w") as f:
                    json.dump(trajres, f, indent=4)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="/horizon-bucket/saturn_v_4dlabel/004_vision/01_users/yingfeng.cai/vggt/trained_model/exp/basemodel/0927/8x8_1e-4_stage2_resumebase_No2/latest.pt",
        help="Path to the trained model weight for loop closure (.pt)"
    )
    parser.add_argument(
        "--config_path_lc",
        type=str,
        default=None,
        help="Path to the trained model config for loop closure (.yaml)"
    )
    parser.add_argument(
        "--pandar_gtpath",
        type=str,
    ...
    parser.add_argument(
        ...
        type=lambda x: x.lower() == 'true', 
        default=False,
        help="whether to use add_loop_closure"
    )
    parser.add_argument(
        '--save_pointcloud', 
        type=lambda x: x.lower() == 'true', 
...
if __name__ == "__main__":
    ...
    if args.debug:
        ...
        args.save_root = "/home/users/yukai.lin/dev/vggt/meshx/data/6v_window16_with_lc159"
        ...
        args.weight_path = "/horizon-bucket/saturn_v_4dlabel/004_vision/01_users/yingfeng.cai/vggt/trained_model/exp/basemodel/extrapool/8x4_2e-5_stage2_resumeno3_chunkdptbp_src63_extrapool31/latest.pt" #window 16 model
        args.weight_path_lc = "/horizon-bucket/saturn_v_4dlabel/004_vision/01_users/yukai.lin/projects/vggt/trained_model/exp/smallmodel/1103/A800_3x8_vggt_alldata_2e-5_stage2_resume_vitlarge_loop_closure_finetune_80k/159.pt"
        # args.weight_path = "/horizon-bucket/saturn_v_4dlabel/004_vision/01_users/yingfeng.cai/vggt/trained_model/exp/smallmodel/1025/L20_8x8_1e-4_vits_woxyzhead_bz24x2_80k_resumefromscratch/latest.pt" # smallmodel
        # args.weight_path = "/horizon-bucket/saturn_v_4dlabel/004_vision/01_users/yukai.lin/projects/vggt/trained_model/exp/smallmodel/1103/l20_vggt_2e-5_stage2_resumesmall_eval_lc_finetune_80k/79.pt" # smallmodel finetune
        # args.weight_path = "/horizon-bucket/saturn_v_4dlabel/004_vision/01_users/yukai.lin/projects/vggt/trained_model/exp/smallmodel/1103/l20_vggt_2e-5_stage2_resumesmall_loop_closure_finetune_80k/79.pt" # smallmodel finetune with lc

        # args.weight_path = "/horizon-bucket/saturn_v_4dlabel/004_vision/01_users/yukai.lin/projects/vggt/trained_model/exp/smallmodel/1103/2x8_l20_2e-5_finetune_overfit_stage2_resumesmall/59.pt" # smallmodel
        args.use_cam_emb = True
        args.mvseq = True
        args.sensors = "6v"
        args.gt_type = "pandar"
        args.add_loop_closure = True
        args.save_pointcloud = False
        args.site_rootpath = "/horizon-bucket/perception-dataprocess/4dlabel/static_visual/users/qingfeng.li/visual_test/pack_list_mode/output/driving_parking_20251019_033425"
        args.pandar_gtpath = "/horizon-bucket/saturn_v_4dlabel/006_4dlabel_hde/000_static_visual/release_eval/visual_eval_gt/pandar_gt_v10.3.0_packlist_parking_newdataset/pandar_b_c_20250823_170228"
        sitesfm = "DZ701/20250711_D/garage__1752235937000__7"
        sitepandar = sitesfm
        ...
        if args.add_loop_closure:
            args.config_path_lc = auto_search_config_yaml(args.weight_path_lc)
    ...
    weight_path_lc = args.weight_path_lc
    config_path_lc = args.config_path_lc
    gt_type = args.gt_type
    mvseq = args.mvseq
    use_cam_emb = args.use_cam_emb
    add_loop_closure = args.add_loop_closure
    ...
    # clip_names = [clip_names[1]]
    for clip_name in clip_names:
        print(f"clip_name: {clip_name}")
    clip_save_root = site_save_root
    CI = ClipInfer(config, site_path, clip_save_root, weight_path, config_path, weight_path_lc, config_path_lc, pandar_site_path, clip_names, camera_names, gt_type, mvseq, use_cam_emb, add_loop_closure, save_pointcloud,low_vram=True)
    CI.use_window = True
    CI.clip_infer()
