...
def _normalize_camera_distortion(D, target_dim: int = None) -> np.ndarray:
    if D is None:
        D = np.zeros((5, 1), dtype=np.float32)
    D = np.asarray(D, dtype=np.float32).reshape(-1)
    target_dim = max(5, int(D.size if target_dim is None else target_dim))
    if D.size < target_dim:
        D = np.pad(D, (0, target_dim - D.size), constant_values=0.0)
    elif D.size > target_dim:
        D = D[:target_dim]
...
class VolumetricVideoDataset(Dataset):
    def __init__(self,
                 ...
                 dataset_name: str = "unknown",  # dataset name for visualization
                 ...
                 max_size: float = -1,  # resize the image to this maximum size, -1 means no resizing
                 align_size: float = 1,  # ensure the image size is divisible by this number
                 center_crop_size: List[int] = [-1, -1],  # center crop image to this size, after resize
                 n_rays: int = -1,  # number of rays to sample from a single image
                 ...
                 meta_data: dict = None,  # maybe cached meta data from the GeneralizableDataset
                 meta_file: str = '__meta.pkl',  # this will be used to load the dataset
                 intri_file: str = 'intri.yml',
                 extri_file: str = 'extri.yml',
                 extri_3ddr_file: str = 'extri_3ddr.yml',
                 use_3ddr: bool = False,
                 aug_3ddr: bool = False,
                 images_dir: str = 'images',
                 cameras_dir: str = 'cameras',  # only when the camera is moving through time
                 extri_type: str = "w2c",  # "w2c" (default) or "c2w"
                 ims_pattern: str = '{frame:06d}.jpg',
                 mks_pattern: str = '{frame:06d}.png',
                 dps_pattern: str = '{frame:06d}.exr',
                 imsize_overwrite: List[int] = [-1, -1],  # overwrite the image size
                 is_bussi_data: bool = False,  # whether the dataset is bussiness data
                 ...
                 sparse_interpolate: bool = False, # whether to use sparse interpolation for depth resizing
                 use_png_depth: bool = False,
                 use_segs: bool = False,
                 use_dynamic_masks: bool = False,
                 depth_mask_th: float = 1.0,  # mask out depth values below this threshold [int(h*depth_mask_th):] = 0
                 
                 # FIXME: what is this?
                 img_list_file: str = None,
                 target_img_list_file: str = None,
                 ...
                 ddp_shard_dataset: bool = False,  # for image based rendering, no sharding for now
                 ...
                 dataset_idx: int = 0,
                 camera_root: str = '',
                 camera_root_base: str = '',
                 ):
        # do checkings
        if n_rays != -1:
            raise ValueError("n_rays is not supported for this dataset")
        if ddp_shard_dataset:
            raise ValueError("ddp_shard_dataset is not supported for this dataset")

        # Global dataset config entries
        self.data_root = data_root
        self.camera_root = camera_root
        self.camera_root_base = camera_root_base
        if not self.camera_root and self.camera_root_base:
            scene_name = os.path.basename(os.path.normpath(self.data_root))
            self.camera_root = join(self.camera_root_base, scene_name)
        self.dataset_name = dataset_name
        self.meta_file = meta_file
        self.intri_file = intri_file
        self.extri_file = extri_file
        self.extri_3ddr_file = extri_3ddr_file
        self.use_3ddr = use_3ddr
        self.aug_3ddr = aug_3ddr
        self.motion_file = motion_file
        self.bodymodel_file = bodymodel_file
        self.canonical_smpl_file = canonical_smpl_file
        self.is_bussi_data = is_bussi_data
        self.extri_type = (extri_type or "w2c").strip().lower()
        ...
        self.mks_pattern = mks_pattern
        self.dps_pattern = dps_pattern
        self.img_list_file = img_list_file
        self.target_img_list_file = target_img_list_file
        self.img_list_paths = None
        self.img_list_by_cam = None
        if self.img_list_file:
            self._load_img_list_selection()
        ...
        # Maybe load the cached meta data (in `.pkl` format for faster loading)
        # Should include `intri`, `extri`, `n_view_total`, `n_frames_total`
        self.meta_data = meta_data
        if self.meta_data is None:
            for _ in range(20):
                try:
                    with open(join(self.data_root, self.meta_file), 'rb') as f:
                        self.meta_data = pickle.load(f)
                    break
                except:
                    if not exists(join(self.data_root, self.meta_file)):
                        log(red(f"Meta data file {join(self.data_root, self.meta_file)} does not exist"))
                        break
                    self.meta_data = None
                    log(red(f"Error loading meta data from {join(self.data_root, self.meta_file)}, retrying in 0.1s"))
                    time.sleep(1.0)
        ...
        if self.img_list_by_cam:
            selected_counts = [len(v) for v in self.img_list_by_cam.values()]
            self.n_view_total = len(self.img_list_by_cam)
            self.n_frames_total = min(selected_counts) if selected_counts else 0
        else:
            if self.meta_data is not None and 'n_view_total' in self.meta_data: self.n_view_total = self.meta_data['n_view_total']
            elif self.view_sample[1] is not None and len(self.view_sample) == 3: self.n_view_total = self.view_sample[1]
            else: self.n_view_total = len([i for i in os.listdir(join(self.data_root, self.images_dir)) if not i.startswith('.')])  # total number of cameras before filtering
            if self.view_sample[1] is not None and len(self.view_sample) == 3: self.n_view_total = min(self.n_view_total, self.view_sample[1])
            # Get the total number of frames
            if self.meta_data is not None and 'n_frames_total' in self.meta_data: self.n_frames_total = self.meta_data['n_frames_total']
            elif self.frame_sample[1] is not None and len(self.frame_sample) == 3: self.n_frames_total = self.frame_sample[1]
            else: self.n_frames_total = min([len(glob(join(self.data_root, self.images_dir, cam, '*'))) for cam in os.listdir(join(self.data_root, self.images_dir)) if not cam.startswith('.')])  # total number of frames before filtering
            if self.frame_sample[1] is not None and len(self.frame_sample) == 3: self.n_frames_total = min(self.n_frames_total, self.frame_sample[1])

            # Some bucket-mounted DTU layouts can sporadically undercount and collapse to 1 frame.
            # If we have a single camera folder with many images, trust the actual image count.
            if int(self.n_frames_total) <= 1:
                try:
                    img_root = join(self.data_root, self.images_dir)
                    cam_dirs = [c for c in os.listdir(img_root) if not c.startswith('.') and os.path.isdir(join(img_root, c))]
                    if len(cam_dirs) == 1:
                        one_cam = cam_dirs[0]
                        valid_ext = ('.jpg', '.jpeg', '.png')
                        real_frames = len([
                            n for n in os.listdir(join(img_root, one_cam))
                            if n.lower().endswith(valid_ext)
                        ])
                        if real_frames > 1:
                            self.n_frames_total = real_frames
                except Exception:
                    pass

        if "/dtu/scan" in self.data_root.replace("\\", "/"):
            log(
                f"[dtu-debug:init] root={self.data_root} "
                f"n_view_total={self.n_view_total} n_frames_total={self.n_frames_total} "
                f"frame_sample={self.frame_sample} view_sample={self.view_sample}"
            )
        ...
        self.use_dynamic_masks = use_dynamic_masks # use dynamic masks
        ...
        self.sparse_interpolate = sparse_interpolate  # whether to use sparse interpolation for depth resizing
        self.use_png_depth = use_png_depth
        self.depth_mask_th = depth_mask_th
        ...
        # Delete meta_data to save memory
        cameras = None
    ...
    def _load_img_list_selection(self):
        if not self.img_list_file or not exists(self.img_list_file):
            return

        img_root = join(self.data_root, self.images_dir)
        by_cam = {}
        with open(self.img_list_file, 'r', encoding='utf-8', errors='ignore') as f:
            for line in f:
                path = line.strip()
                if not path:
                    continue
                ...
                try:
                    rel = os.path.relpath(real, img_root)
                except ValueError:
                    continue
                if rel.startswith('..'):
                    continue
                ...
                if len(parts) < 2:
                    continue
                cam = parts[0]
                by_cam.setdefault(cam, []).append(real)

        if not by_cam:
            raise ValueError(f'img_list_file={self.img_list_file} contains no valid images under {img_root}')

        for cam, paths in by_cam.items():
            by_cam[cam] = sorted(dict.fromkeys(paths))
        self.img_list_by_cam = by_cam
        self.img_list_paths = [p for paths in by_cam.values() for p in paths]
    ...
    def load_paths(self):
        ...
        if self.img_list_by_cam:
            ims = [np.asarray(self.img_list_by_cam[cam])[:self.n_frames_total] for cam in self.camera_names]
        elif not self.is_bussi_data:
            ...
            for cam in self.camera_names:
                cam_infos = self.cameras[cam]
                cam_file_idx[cam] = [i['file_idx'] for i in cam_infos]
            ims = [[join(self.data_root, self.images_dir, cam, self.ims_pattern.format(frame=i)) for i in cam_file_idx[cam]] for cam in self.camera_names]
        if self.ims_pattern is None:
            ...
            if len(ims) <= 0 or len(ims[0]) <= 0:
                raise ValueError(f"No images found in {join(self.data_root, self.images_dir)}")
        ...
        if self.use_masks:
            if self.mks_pattern is not None:
                mask_ext = self.mks_pattern.split('.')[-1]
                self.mks = np.asarray([im.replace(self.images_dir, self.masks_dir).replace(im.split('.')[-1], mask_ext) for im in self.ims.ravel()]).reshape(self.ims.shape)
            else:
        ...
        if self.use_depths:
            if self.dps_pattern is not None:
                depth_ext = self.dps_pattern.split('.')[-1]
                self.dps = np.asarray([im.replace(self.images_dir, self.depths_dir).replace(im.split('.')[-1], depth_ext) for im in self.ims.ravel()]).reshape(self.ims.shape)
            else:
                if self.use_png_depth:
                    self.dps = np.asarray([im.replace(self.images_dir, self.depths_dir).replace('.jpg', '.png') for im in self.ims.ravel()]).reshape(self.ims.shape)
                else:
                ...
                if not exists(self.dps[0, 0]):
                    if self.use_png_depth:
                        self.dps = np.asarray([dp.replace('.png', 'png') for dp in self.dps.ravel()]).reshape(self.dps.shape)
                    else:
    ...
    def load_bytes(self):
        ...
        dist_dim = int(np.prod(self.Ds.shape[2:]))
        dist_mask = torch.as_tensor(self.dist_mask, dtype=self.Ds.dtype)
        if dist_mask.numel() < dist_dim:
            dist_mask = torch.cat([dist_mask, torch.ones(dist_dim - dist_mask.numel(), dtype=dist_mask.dtype)])
        elif dist_mask.numel() > dist_dim:
            dist_mask = dist_mask[:dist_dim]
        self.Ds = self.Ds.reshape(*self.Ds.shape[:2], dist_dim) * dist_mask  # some of the distortion parameters might need some manual massaging
        ...
        if self.use_masks:
            self.mks_bytes, self.Ks, self.Hs, self.Ws = \
                load_resize_undist_ims_bytes(self.mks, ori_Ks.numpy(), ori_Ds.numpy(), ratio, self.max_size, self.align_size, self.center_crop_size,
                                             f'Loading mask bytes for {blue(self.mks_dir)} {magenta(self.split.name)}',
                                             decode_flag=cv2.IMREAD_GRAYSCALE, dist_opt_K=self.dist_opt_K, encode_ext=self.encode_ext, interpolation=cv2.INTER_NEAREST,
                                             num_workers=self.dataloading_workers)  # will for a grayscale read from bytes
        ...
        if self.use_bkgds:
            self.bgs_bytes, _, _, _ = \
                load_resize_undist_ims_bytes(self.bgs, ori_Ks[:, 0].numpy(), ori_Ds[:, 0].numpy(), ratio, self.max_size, self.align_size, self.center_crop_size,
                                             f'Loading bkgd bytes for {blue(self.bgs_dir)} {magenta(self.split.name)}',
                                             dist_opt_K=self.dist_opt_K, encode_ext=self.encode_ext, interpolation=cv2.INTER_AREA,
                                             num_workers=self.dataloading_workers)
        ...
        if self.use_depths:  # TODO: implement HDR loading
            self.dps_bytes, self.Ks, self.Hs, self.Ws = \
                load_resize_undist_ims_bytes(self.dps, ori_Ks.numpy(), ori_Ds.numpy(), ratio, self.max_size, self.align_size, self.center_crop_size,
                                             f'Loading dpts bytes for {blue(self.dps_dir)} {magenta(self.split.name)}',
                                             decode_flag=cv2.IMREAD_UNCHANGED, dist_opt_K=self.dist_opt_K, encode_ext='.exr', interpolation=cv2.INTER_NEAREST, sparse_interpolate=self.sparse_interpolate,
                                             use_png_depth=self.use_png_depth, depth_mask_th=self.depth_mask_th, num_workers=self.dataloading_workers)  # will for a grayscale read from bytes

        # Maybe load normal images here
        if self.use_normals:
            self.nms_bytes, self.Ks, self.Hs, self.Ws = \
                load_resize_undist_ims_bytes(self.nms, ori_Ks.numpy(), ori_Ds.numpy(), ratio, self.max_size, self.align_size, self.center_crop_size,
        ...
        self.ims_bytes, self.Ks, self.Hs, self.Ws = \
            load_resize_undist_ims_bytes(self.ims, ori_Ks.numpy(), ori_Ds.numpy(), ratio, self.max_size, self.align_size, self.center_crop_size,
    ...
    def __load_cameras(self, extri_file):
        ...
        # Maybe use the cached pkl files or cached meta data
        camera_root = self.camera_root or self.data_root
        camera_dir = join(camera_root, self.cameras_dir)
        selected_camera_names = set(self.img_list_by_cam.keys()) if self.img_list_by_cam else None
        if self.meta_data is not None:
            # Get the name of the intri and extri files, maybe a better way to handle this
            intri_name, extri_name = self.intri_file.split('.')[0], extri_file.split('.')[0]

            # Multiview dataset loading, need to expand, will have redundant information
            if intri_name in self.meta_data and extri_name in self.meta_data:
                cameras = read_camera(self.meta_data[intri_name], self.meta_data[extri_name], use_dict=True)
                self.camera_names = np.asarray(sorted([
                    k for k in cameras.keys()
                    if selected_camera_names is None or k in selected_camera_names
                ]))  # NOTE: sorting camera names
                return dotdict({k: [cameras[k] for i in range(self.n_frames_total)] for k in self.camera_names})

            # Monocular dataset loading, each camera has a separate folder, `**/intri` and `**/extri`
            elif any(intri_name in key for key in self.meta_data.keys()) and any(extri_name in key for key in self.meta_data.keys()):
                self.camera_names = [i.split('/')[0] for i in self.meta_data.keys() if i.endswith(intri_name)]
                cameras = dotdict({
                    k: self._select_camera_time_slices(sorted(
                        read_camera(self.meta_data[f'{k}/{intri_name}'], self.meta_data[f'{k}/{extri_name}'], use_dict=True).items()
                    )) for k in self.camera_names
                })
                try:
                    max_loaded_frames = max((len(v) for v in cameras.values()), default=0)
                except Exception:
                    max_loaded_frames = 0
                if int(self.n_frames_total) > 1 and max_loaded_frames <= 1:
                    camera_dir_entries = self._probe_camera_dir(camera_dir)
                    if camera_dir_entries is not None:
                        log(yellow(
                            f"Meta cameras look degenerated (max_loaded_frames={max_loaded_frames}, "
                            f"n_frames_total={self.n_frames_total}), fallback to on-disk camera files: "
                            f"{camera_dir}"
                        ))
                        self.camera_names = np.asarray(camera_dir_entries)  # NOTE: sorting here is very important!
                        return dotdict({
                            k: self._select_camera_time_slices(sorted(
                                read_camera(join(camera_dir, k, self.intri_file),
                                            join(camera_dir, k, extri_file)).items()
                            )) for k in self.camera_names
                        })
                return cameras
            else:
                log(yellow(f"Could not find {intri_name} or {extri_name} in {self.meta_data}"))
                log(yellow("Use the default camera loading method"))

        # Multiview dataset loading, need to expand, will have redundant information
        if exists(join(camera_root, self.intri_file)) and exists(join(camera_root, extri_file)):
            cameras = read_camera(join(camera_root, self.intri_file), join(camera_root, extri_file))
            self.camera_names = np.asarray(sorted([
                k for k in cameras.keys()
                if selected_camera_names is None or k in selected_camera_names
            ]))  # NOTE: sorting camera names
        ...
        raise NotImplementedError(f'Could not find {{{self.intri_file},{extri_file}}} or {self.cameras_dir} directory in {camera_root}, check your dataset configuration or use NoopDataset')

    @staticmethod
    def _probe_camera_dir(camera_dir: str, retries: int = 10, sleep_s: float = 0.2):
        last_error = None
        for _ in range(max(1, int(retries))):
            try:
                if exists(camera_dir) and os.path.isdir(camera_dir):
                    return sorted([k for k in os.listdir(camera_dir) if not k.startswith('.')])
            except OSError as e:
                last_error = e
            time.sleep(float(sleep_s))

        if last_error is not None:
            log(yellow(f"Error probing camera directory {camera_dir}: {last_error}"))
        return None

    def _select_camera_time_slices(self, cam_items):
        cams = [v[1] for v in cam_items]
        if len(cams) <= 1:
            return cams
        n_keep = max(1, int(self.n_frames_total))
        # Some bucket mounts can transiently undercount image files (n_frames_total=1)
        # even though camera yml stores the full sequence. Keep full sequence by default.
        if n_keep <= 1 and os.environ.get("EVC_FORCE_MONO_FULL_FRAMES", "1") == "1":
            return cams
        return cams[:min(n_keep, len(cams))]

    def load_cameras(self):
        self.cameras = self.__load_cameras(self.extri_file)
        if self.use_3ddr:
            self.cameras_3ddr = self.__load_cameras(self.extri_3ddr_file)
        ...
        dist_dim = max(
            5,
            max(
                int(np.asarray(cam.D if cam.D is not None else np.zeros((5, 1))).size)
                for k in self.camera_names
                for cam in self.cameras[k]
            )
        )
        self.Ds = torch.as_tensor([[_normalize_camera_distortion(cam.D, dist_dim) for cam in self.cameras[k]] for k in self.camera_names], dtype=torch.float)  # V, F, D, 1
        ...
        if "/dtu/scan" in self.data_root.replace("\\", "/"):
            cam_frames = len(next(iter(self.cameras.values()))) if len(self.cameras) else 0
            log(
                f"[dtu-debug:load] root={self.data_root} cams={len(self.camera_names)} "
                f"cam_frames={cam_frames} Hs={tuple(self.Hs.shape)} "
                f"frame_sample={self.frame_sample}"
            )

        if os.environ.get("EVC_DEBUG_CAM_SHAPES", "0") == "1":
            log(
                f"[cam-debug] data_root={self.data_root} "
                f"cams={len(self.camera_names)} Hs={tuple(self.Hs.shape)} "
                f"Ks={tuple(self.Ks.shape)} Rs={tuple(self.Rs.shape)} Ts={tuple(self.Ts.shape)}"
            )

        # Some datasets (e.g. raw RE10K metadata) provide camera-to-world matrices;
        # allow interpreting extri.yml as c2w and invert them to w2c here.
        if self.extri_type in {"c2w", "camera_to_world", "cam2world"}:
            c2ws = torch.cat([self.Rs, self.Ts], dim=-1)  # V, F, 3, 4
            self.w2cs = affine_inverse(c2ws)  # V, F, 3, 4
            self.c2ws = c2ws
            self.Rs = self.w2cs[..., :3, :3]
            self.Ts = self.w2cs[..., :3, 3:]
        else:
        ...
        if self.use_3ddr:
            self.Rs_3ddr = torch.as_tensor([[cam.R for cam in self.cameras_3ddr[k]] for k in self.camera_names], dtype=torch.float)  # V, F, 3, 3
            self.Ts_3ddr = torch.as_tensor([[cam.T for cam in self.cameras_3ddr[k]] for k in self.camera_names], dtype=torch.float)  # V, F, 3, 1
            self.w2cs_3ddr = torch.cat([self.Rs_3ddr, self.Ts_3ddr], dim=-1)  # V, F, 3, 4
            self.c2ws_3ddr = affine_inverse(self.w2cs_3ddr)  # V, F, 3, 4
    ...
    def select_cameras(self):
        ...
        if "/dtu/scan" in self.data_root.replace("\\", "/"):
            log(
                f"[dtu-debug:select-pre] root={self.data_root} Ks={tuple(self.Ks.shape)} "
                f"frame_inds_len={len(frame_inds)} frame_sample={self.frame_sample}"
            )
        ...
        if "/dtu/scan" in self.data_root.replace("\\", "/"):
            log(
                f"[dtu-debug:select-post] root={self.data_root} Ks={tuple(self.Ks.shape)} "
                f"n_views={self.n_views} n_latents={self.n_latents}"
            )
        
        if self.use_3ddr:
            self.cameras_3ddr = dotdict({k: [self.cameras_3ddr[k][int(i)] for i in frame_inds] for k in self.camera_names})  # reloading
            self.Rs_3ddr = self.Rs_3ddr[view_inds][:, frame_inds]
            self.Ts_3ddr = self.Ts_3ddr[view_inds][:, frame_inds]
            self.w2cs_3ddr = self.w2cs_3ddr[view_inds][:, frame_inds]
            self.c2ws_3ddr = self.c2ws_3ddr[view_inds][:, frame_inds]
    ...
    def get_image_from_bytes(self, view_index: int, latent_index: int):
        ...
        # Fetch the image height, width and the intrinsic matrix for consistency with the disk version
        H, W, K = self.Hs[view_index][latent_index], self.Ws[view_index][latent_index], self.Ks[view_index][latent_index]

        return rgb, msk, wet, dpt, bkg, norm, H, W, K
    ...
    def get_image_from_disk(self, view_index: int, latent_index: int, output: dotdict = None):
        ...
        ratio = self.imsize_overwrite if self.imsize_overwrite[0] > 0 else self.ratio  # maybe force size, or maybe use ratio to resize
        ratio = ratio[view_index] if isinstance(ratio, list) else ratio

        if self.use_masks:
            mk = self.mks[view_index][latent_index]
            mk, _, _, _ = load_resize_undist_im_bytes(mk, K.numpy(), D.numpy(), ratio, self.max_size, self.align_size, self.center_crop_size, decode_flag=cv2.IMREAD_GRAYSCALE, dist_opt_K=self.dist_opt_K, encode=False, interpolation=cv2.INTER_NEAREST)
            mk = mk[..., None] if len(mk.shape) == 2 else mk

        # Maybe load background images here
        if self.use_bkgds:
            bg = self.bgs[view_index]
            bg, _, _, _ = load_resize_undist_im_bytes(bg, K.numpy(), D.numpy(), ratio, self.max_size, self.align_size, self.center_crop_size, dist_opt_K=self.dist_opt_K, encode=False, interpolation=cv2.INTER_AREA)

        # Maybe load depth images here, using HDR
        if self.use_depths:
            dp = self.dps[view_index][latent_index]
            dp, _, _, _ = load_resize_undist_im_bytes(dp, K.numpy(), D.numpy(), ratio, self.max_size, self.align_size, self.center_crop_size, decode_flag=cv2.IMREAD_UNCHANGED, dist_opt_K=self.dist_opt_K, encode=False, interpolation=cv2.INTER_NEAREST, 
                                                      sparse_interpolate=self.sparse_interpolate, use_png_depth=self.use_png_depth, depth_mask_th=self.depth_mask_th)  # will for a grayscale read from bytes
            dp = dp[..., None] if len(dp.shape) == 2 else dp

        # Maybe load normal images here
        if self.use_normals:
            nm = self.nms[view_index][latent_index]
            nm, _, _, _ = load_resize_undist_im_bytes(nm, K.numpy(), D.numpy(), ratio, self.max_size, self.align_size, self.center_crop_size, dist_opt_K=self.dist_opt_K, encode=False, interpolation=cv2.INTER_AREA)


        if self.use_segs:
            seg = os.path.realpath(self.ims[view_index][latent_index]).replace("keyframe", "seg").replace(".jpg", ".png")
            seg, _, _, _ = load_resize_undist_im_bytes(seg, K.numpy(), D.numpy(), ratio, self.max_size, self.align_size, self.center_crop_size, decode_flag=cv2.IMREAD_UNCHANGED, dist_opt_K=self.dist_opt_K, encode=False, interpolation=cv2.INTER_NEAREST, 
                                                      sparse_interpolate=self.sparse_interpolate, use_segs=self.use_segs, depth_mask_th=self.depth_mask_th)  # will for a grayscale read from bytes
            seg = seg[..., None] if len(seg.shape) == 2 else seg
            
            
        if self.use_dynamic_masks and self.use_segs:
            dynamic_mk = os.path.realpath(self.ims[view_index][latent_index]).replace("keyframe", "dynamic_mask").replace(".jpg", ".png")
            if not os.path.exists(dynamic_mk):
                dynamic_mk = torch.full(size=seg.shape[:2], fill_value=torch.nan, dtype=torch.float32)
            else:
                dynamic_mk, _, _, _ = load_resize_undist_im_bytes(dynamic_mk, K.numpy(), D.numpy(), ratio, self.max_size, self.align_size, self.center_crop_size, decode_flag=cv2.IMREAD_UNCHANGED, dist_opt_K=self.dist_opt_K, encode=False, interpolation=cv2.INTER_NEAREST, 
                                                        sparse_interpolate=self.sparse_interpolate, use_dynamic_masks=self.use_dynamic_masks, depth_mask_th=self.depth_mask_th)  # will for a grayscale read from bytes
                dynamic_mk = dynamic_mk[..., None] if len(dynamic_mk.shape) == 2 else dynamic_mk

                
        # Image pre cacheing (from disk to memory)
        im = self.ims[view_index][latent_index]
        im, K, H, W = load_resize_undist_im_bytes(im, K.numpy(), D.numpy(), ratio, self.max_size, self.align_size, self.center_crop_size, dist_opt_K=self.dist_opt_K, encode=False, interpolation=cv2.INTER_AREA)
        ...
        if 'dynamic_mk' not in locals(): dynamic_mk = None
        else: dynamic_mk = torch.as_tensor(dynamic_mk)
        if 'seg' not in locals(): seg = None
        else: seg = torch.as_tensor(seg)

        return im, mk, dynamic_mk, seg, wt, dp, bg, nm, H, W, torch.as_tensor(K)
    ...
    def get_metadata(self, index):
        ...
        if self.use_loaded_time:
            ...
            # NOTE:
            # During standalone eval we usually only provide `val_dataloader_cfg`.
            # The legacy helper `parse_n_frames_views()` reads from global
            # `cfg.dataloader_cfg.dataset_cfg` and may fall back to an empty
            # data_root, causing repeated "path: images" probes and wrong time
            # normalization. Always derive t from the current dataset instance.
            if self.split == DataSplit.TRAIN:
                meta.t = self.frame_to_t(frame_index)  # for duration of 1.0, this is a no-op
            else:
                if len(self.frame_sample) != 3:
                    n_frames = max(int(self.n_latents), 1)
                    meta.t = latent_index / n_frames
                else:
                    n_frames = max(int(self.n_frames_total), 1)
                    meta.t = (frame_index - self.frame_sample[0]) / n_frames
    ...
    def get_ground_truth(self, index):
        ...
        rgb, msk, wet, dpt, bkg, norm, _, _, _ = self.get_image(output.view_index, output.latent_index, output)  # H, W, 3
        H, W = rgb.shape[:2]
...
# a hacky function to parse the number of frames and views
def parse_n_frames_views():
    ...
    return n_frames, n_views, frame_sample, view_sample
