...
from easyvolcap.utils.depth_utils import depth_curve_fn, conf_curve_fn
...
class VolumetricVideoVisualizer:  # this should act as a base class for other types of visualizations (need diff dataset)
    def __init__(self,
                 ...
                 pool_limit: int = 1,  # maximum number of pending tasks in the thread pool, keep this small to avoid using too much resource
                 video_fps: int = 60,
                 verbose: bool = True,
                 ...
                 cnf_cm: str = 'linear' if args.type != 'gui' else 'linear',  # looks good
                 ):
        super().__init__()
    ...
    def generate_type(self, output: dotdict, batch: dotdict, type: Visualization = Visualization.RENDER):
        # NOTE: To make the visualization more general, we will consider the following assumptions:
        # 1. We assume all the variables to be visualized share the same dimension format (B, P, C) or (B, S, P, C)
        # 2. We concatenate all the source images in a batch along the width dimension to form a single image if necessary
        ...
        if type == Visualization.NORMAL:
            def norm_curve_fn(norm):
                norm = normalize(norm)
                # TODO: ... hack, need to be polished
                if norm.ndim > 3: norm = norm @ batch.w2cs[..., :3, :3].mT
                else: norm = norm @ batch.R.mT
                norm[..., 1] *= -1
                norm[..., 2] *= -1
                norm = norm * 0.5 + 0.5
            ...
            if self.dpt_curve == 'linear':
                ...
                if output.dpt_map.ndim > 3:
                    img = torch.stack([
                        depth_curve_fn(
                            dpt,
                            mask=msk,
                            cm=self.dpt_cm
                        ) for dpt, msk in zip(output.dpt_map.unbind(1), batch.msk.unbind(1))
                    ], dim=1)
                else:
                    img = depth_curve_fn(output.dpt_map, mask=batch.msk, cm=self.dpt_cm)
                    ...
                    if batch.dpt.ndim > 3:
                        img_gt = torch.stack([
                            depth_curve_fn(
                                ...
                                mask=msk,
                                cm=self.dpt_cm
                            ) for dpt, msk in zip(batch.dpt.unbind(1), batch.msk.unbind(1))
                        ], dim=1)
                    else:
                        img_gt = depth_curve_fn(batch.dpt, mask=batch.msk, cm=self.dpt_cm)
                # img_gt = (img_gt - 0.5) * self.dpt_mult + 0.5
                img_gt = img_gt * self.dpt_mult

        elif type == Visualization.DPTCONF:
            img = output.dpt_cnf
            if img.ndim > 3:
                img = torch.stack([
                    conf_curve_fn(
                        cnf,
                        cm=self.cnf_cm
                    ) for cnf in output.dpt_cnf.unbind(1)
                ], dim=1)
            else:
                img = conf_curve_fn(output.dpt_cnf, cm=self.cnf_cm)

        elif type == Visualization.XYZCONF:
            img = output.xyz_cnf
            if img.ndim > 3:
                img = torch.stack([
                    conf_curve_fn(
                        cnf,
                        cm=self.cnf_cm
                    ) for cnf in output.xyz_cnf.unbind(1)
                ], dim=1)
            else:
                img = conf_curve_fn(output.xyz_cnf, cm=self.cnf_cm)
            ...
            img = output.flo_map
            C, sh = img.shape[-1], img.shape[:-2]
            H, W = batch.meta.H[0].item(), batch.meta.W[0].item()
            img = img.view(-1, H, W, C).float()  # (B * S, H, W, 2)
            img = img.permute(0, 3, 1, 2)  # (B * S, 2, H, W)
            img = flow_to_image(img).view(img.shape[0], -1, H * W).permute(0, 2, 1)  # (B * S, P, 3)
            img = img.float() / 255.0  # (B * S, P, 3)
            img = img.reshape(sh + img.shape[-2:])  # (B, P, 3) or (B, S, P, 3)
            if 'flow_weight' in batch:
                img = img * batch.flow_weight.view(sh + (H * W, 1))
            if self.store_ground_truth and 'flow' in batch:
                img_gt = batch.flow.view(-1, H, W, C).float()  # (B * S, H, W, 2)
                img_gt = img_gt.permute(0, 3, 1, 2)  # (B * S, 2, H, W)
                img_gt = flow_to_image(img_gt).view(img_gt.shape[0], -1, H * W).permute(0, 2, 1)  # (B * S, P, 3)
                img_gt = img_gt.float() / 255.0  # (B * S, P, 3)
                img_gt = img_gt.reshape(sh + img_gt.shape[-2:])  # (B, S, P, 3)
                if 'flow_weight' in batch:
                    img_gt = img_gt * batch.flow_weight.view(sh + (H * W, 1))  # (B, P, 3) or (B, S, P, 3)
        ...
        # ... implement more
            ...
            if 'src_inps' in batch:
                img = batch.src_inps.permute(0, 1, 3, 4, 2)  # (B, S, C, H, W) -> (B, S, H, W, C)
                img = img.reshape(img.shape[:2] + (-1, img.shape[-1]))  # (B, S, P, C)
            elif 'rgb' in batch:
                img = batch.rgb  # (B, S, P, C)
            else:
                raise ValueError('No src_inps or rgb in batch')
        
        elif type == Visualization.XYZ:
            img = output.xyz_map
            msk = batch.msk if batch.msk.shape[-1] != 1 else batch.msk[..., 0]
            if msk.sum() > 0:
                k = int(img[msk].numel() * 0.01)
                n = img[msk].ravel().topk(k, largest=False)[0].max()  # a simple version of percentile
                f = img[msk].ravel().topk(k, largest=True)[0].min()  # a simple version of percentile
                img = (img - n) / (f - n)
            else:
                img = img.new_zeros_like(img)
        ...
        C, sh = img.shape[-1], img.shape[:-2]
        H, W = batch.meta.H[0].item(), batch.meta.W[0].item()
        img = img.view(sh + (H, W, C)).float()
        if img_gt is not None: img_gt = img_gt.view(sh + (H, W, C)).float()
        if img_error is not None: img_error = img_error.view(sh + (H, W, C)).float()

        if len(sh) > 1:
            img = torch.cat([img[:, i] for i in range(img.shape[1])], dim=-2)  # (B, H, W * S, C)
            if img_gt is not None: img_gt = torch.cat([img_gt[:, i] for i in range(img_gt.shape[1])], dim=-2)
            if img_error is not None: img_error = torch.cat([img_error[:, i] for i in range(img_error.shape[1])], dim=-2)
        ...
        if self.uncrop_output_images:  # necessary for GUI applications
            if 'orig_h' in batch.meta:  # !: BATCH: Removed
                ...
                img_full = img.new_zeros(sh + (H, W, C))  # original size
                img_full[..., y:y + h, x:x + w, :] = img
                img = img_full
                if img_gt is not None:
                    img_gt_full = img_gt.new_zeros(sh + (H, W, C))  # original size
                    img_gt_full[..., y:y + h, x:x + w, :] = img_gt
                    img_gt = img_gt_full
                if img_error is not None:
                    img_error_full = img_error.new_zeros(sh + (H, W, C))  # original size
    ...
    def visualize_type(self, output: dotdict, batch: dotdict, type: Visualization = Visualization.RENDER):
        ...
        for i in range(len(imgs)):
            ...
            img_paths.append(join(self.result_dir, f'{batch.meta.iter:08d}', img_path))
            img_arrays.append(img.detach().to('cpu', non_blocking=True))  # start moving
            if img_gts is not None:
                img_paths.append(join(self.result_dir, f'{batch.meta.iter:08d}', img_gt_path))
                img_arrays.append(img_gt.detach().to('cpu', non_blocking=True))  # start moving
            if img_errors is not None:
                img_paths.append(join(self.result_dir, f'{batch.meta.iter:08d}', img_error_path))
