        ...
        for i, r in self.milestones:
            if self.iter >= i:
                ...
                if hasattr(self.runner, 'dataloader') and self.runner.dataloader is not None:
                    if hasattr(self.runner.dataloader, 'dataset'):
                        self.runner.dataloader.dataset.render_ratio = r
                    if hasattr(self.runner.dataloader, 'datasets'):
                        for dataset in self.runner.dataloader.datasets:
                            dataset.render_ratio = r
                if hasattr(self.runner, 'val_dataloader') and self.runner.val_dataloader is not None:
                    if hasattr(self.runner.val_dataloader, 'dataset'):
                        self.runner.val_dataloader.dataset.render_ratio = r
                    if hasattr(self.runner.val_dataloader, 'datasets'):
                        for dataset in self.runner.val_dataloader.datasets:
                            dataset.render_ratio = r
        ...
        for i, r in self.milestones:
            if self.iter >= i:
                ...
                if hasattr(self.runner, 'dataloader') and self.runner.dataloader is not None:
                    ...
                    if hasattr(self.runner.dataloader, 'datasets'):
                        for dataset in self.runner.dataloader.datasets:
                            dataset.render_center_crop_ratio = r
                if hasattr(self.runner, 'val_dataloader') and self.runner.val_dataloader is not None:
                    if hasattr(self.runner.val_dataloader, 'dataset'):
