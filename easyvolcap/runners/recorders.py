...
class NoopRecorder:
    """不落盘 recorder：用于大规模推理/评测时避免 Tensorboard/配置 dump 占用磁盘。"""

    def __init__(self, *args, **kwargs):
        self.record_dir = kwargs.get("record_dir", "")
        self.epoch = 0
        self.iter = 0
...
class TensorboardRecorder:
    def __init__(self,
        ...
        if log_to_file and log_filename:
            log_path = join(record_dir, log_filename)
            set_log_file(log_path, mode="a" if resume else "w")
            log(f"Saving training log to {blue(log_path)}")
        ...
        if record_config:
            # NOTE: cfg.runner 会在 Runner 构造阶段被挂到全局 cfg 上（用于某些 hack）。
            # 当一个进程里连续多次构建 runner（如 clip_infer_official 分段推理）时，
            # 下一次构建 recorder 会在旧的 cfg.runner 仍存在的情况下 dump cfg，
            # 进而触发 YAML 序列化异常（例如 ValueError: dictionary update sequence ...）。
            with WithoutKey(['eglctx', 'runner'], cfg):
                cfg_file = join(record_dir, f'{int(datetime.now().timestamp())}.yaml')
    ...
    def update_image_stats(self, image_stats: dotdict):
        ...
        for k, v in image_stats.items():
            if k.endswith('.png') or k.endswith('.jpg') or k.endswith('.jpeg') or k.endswith('.webp'):
