...
from easyvolcap.utils.base_utils import default_dotdict, dotdict, DoNothing, is_on_aidi_cluster
...
slim_width = None if not is_on_aidi_cluster() else 180
verbose_width = None
slim_log_time = True
slim_log_path = True
slim_time_format = '%m-%d %H:%M:%S.%f'
# slim_time_format = ''
...
log_file_console = None
log_file_path = None


def set_log_file(path: Optional[str], mode: str = "a"):
    global log_file_handle, log_file_console, log_file_path
    ...
    if path:
        ...
        log_file_handle = open(path, mode, encoding="utf-8")
        log_file_console = Console(
            file=log_file_handle,
            soft_wrap=True,
            tab_size=4,
            log_time_format=verbose_time_format,
            width=verbose_width,
            color_system=None,
...
# overwrite the exists function
def exists(path):
    try:
        os.lstat(path)
    except (OSError, ValueError) as e:
        log(f"os.stat error, path: {path}, hostname: {hostname()}, error: {e}")
        return False
    return True
...
def post_mortem(*args, **kwargs):
    ...
    if breakpoint_disabled:
        return
    disable_pm = os.getenv("EVC_NO_POST_MORTEM", "")
    if disable_pm.lower() in ("1", "true", "yes", "y", "on"):
        return
    if not os.path.isdir("/job_data"):
...
def log(*stuff,
    ...
    prefix = None
    if not (no_prefix or not verbose_log):
        prefix = get_log_prefix(back + 1, module_color, func_color)

    if prefix is None:
        writer.log(*stuff, _stack_offset=2, **kwargs)
    else:
        writer.log(prefix, *stuff, _stack_offset=2, **kwargs)

    if file is None and log_file_console is not None:
        log_file_console._log_render.time_format = writer._log_render.time_format
        if prefix is None:
            log_file_console.log(*stuff, _stack_offset=2, **kwargs)
        else:
            log_file_console.log(prefix, *stuff, _stack_offset=2, **kwargs)
        if log_file_handle is not None:
...
class Timer:
    ...
    def record(self, event: str = '', log_interval: float = -1):
        ...
        if self.record_to_file and event:
            ...
            with open(join(self.record_dir, f'timing.json'), 'w') as f:
