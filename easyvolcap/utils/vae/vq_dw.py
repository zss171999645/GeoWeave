...
@dataclass
class ModelArgs:
    codebook_len: int = 16384
    codebook_dim: int = 16
    codebook_l2_norm: bool = True
    codebook_show_usage: bool = True
    commit_loss_beta: float = 0.25
    entropy_loss_ratio: float = 0.0
    # condition_frames: int = 15
...
class VideoVQModel(nn.Module):
    def __init__(self, config: ModelArgs):
        ...
        self.quantize = VectorQuantizer(config.codebook_len, config.codebook_dim, 
                                        config.commit_loss_beta, config.entropy_loss_ratio,
                                        config.codebook_l2_norm, config.codebook_show_usage)
        self.quant_conv = nn.Conv2d(config.z_channels, config.codebook_dim, 1)
        self.post_quant_conv = nn.Conv2d(config.codebook_dim, config.z_channels, 1)
    ...
    def spatial_temporal_blocks(self, x):
        ...
        matrix = torch.tril(torch.ones(self.condition_frames, self.condition_frames)) # input frames [1, 0, 0; 1, 1, 0; 1, 1, 1]
        ...
        b, f, l, c = xx.shape
        xx = rearrange(xx, 'b f l c -> (b l) f c') # l = h*w
