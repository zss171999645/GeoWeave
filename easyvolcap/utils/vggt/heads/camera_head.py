...
from easyvolcap.utils.vggt.layers import Mlp
from easyvolcap.utils.vggt.layers.block import Block
from easyvolcap.utils.vggt.heads.head_act import activate_pose_v2


class CustomSequential(nn.Sequential):
    def forward(self, x, *inputs, **kwargs):
        for module in self:
            x = module(x, *inputs, **kwargs)
        return x
...
class CameraHead(nn.Module):
    ...
    def __init__(
        ...
        print(f"CameraHead target_dim: {self.target_dim}")
    ...
    def predict_pose(self, pred_pose_enc: torch.Tensor) -> torch.Tensor:
        activated_pose = activate_pose_v2(
            pred_pose_enc,
            self.nonlinear_def,
        )
