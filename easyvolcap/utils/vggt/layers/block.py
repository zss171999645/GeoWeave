...
from easyvolcap.utils.vggt.layers.attention import Attention
from easyvolcap.utils.vggt.layers.drop_path import DropPath
from easyvolcap.utils.vggt.layers.layer_scale import LayerScale
...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, pos=None, cams=None, cam_drop=False) -> Tensor:
        def attn_residual_func(x: Tensor, pos=None, scale_msa=None, shift_msa=None) -> Tensor:
            if not self.use_cam_emb or cams is None:
                ...
                return self.ls1(self.attn(
                    func(self.norm1(x), shift_msa, scale_msa),
                    pos=pos
                ))
        ...
        def ffn_residual_func(x: Tensor, scale_mlp=None, shift_mlp=None) -> Tensor:
            if not self.use_cam_emb or cams is None:
                ...
                return self.ls2(self.mlp(
                    func(self.norm2(x), shift_mlp, scale_mlp)
                ))
        ...
        if self.training and self.sample_drop_ratio > 0.1:
            ...
            x = x + self.drop_path1(attn_residual_func(
                x,
                pos=pos,
                scale_msa=scale_msa,
                shift_msa=shift_msa,
            ))
            x = x + self.drop_path2(ffn_residual_func(
                x,
                scale_mlp=scale_mlp,
                shift_mlp=shift_mlp,
            ))
