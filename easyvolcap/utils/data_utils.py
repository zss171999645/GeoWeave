...
from typing import Tuple, Union, List, Dict, Any, Generator
...
class Visualization(Enum):
    ...
    DPTCONF = auto()  # confidence map for depth estimation
    XYZCONF = auto()  # confidence map for xyz estimation
    XYZ = auto()  # xyz estimation
...
def remove_batch(batch) -> Union[torch.Tensor, np.ndarray]:
    if isinstance(batch, list):
        for i, v in enumerate(batch):
            batch[i] = remove_batch(v)
        return batch
    elif isinstance(batch, tuple):
        return tuple(remove_batch(v) for v in batch)
    elif isinstance(batch, dict):
        for k, v in batch.items():
            batch[k] = remove_batch(v)
    elif isinstance(batch, (torch.Tensor, np.ndarray)) and len(batch.shape):  # numpy and others
        batch = batch[0]
    return batch


def remove_batch_nomod(batch) -> Union[torch.Tensor, np.ndarray]:
    if isinstance(batch, (tuple, list)):
        batch = [remove_batch_nomod(b) for b in batch]  # recursive
    elif isinstance(batch, dict):
        batch = dotdict({k: remove_batch_nomod(v) for k, v in batch.items()})  # recursive
    elif isinstance(batch, (torch.Tensor, np.ndarray)) and len(batch.shape) > 0:  # numpy and others
        batch = batch[0]
    return batch
...
def to_precision(batch: Dict[str, Any], dtype: torch.dtype, ignore_list: bool = False) -> Dict[str, Any]:
    """
    Recursively iterates over all keys in a nested dictionary and
    and converts all torch.Tensor values to the specified dtype.

    NOTE: in-place operation to avoid information loss
    """
    if isinstance(batch, dict):
        for k, v in batch.items():
            batch[k] = to_precision(v, dtype, ignore_list)
    elif isinstance(batch, list) and not ignore_list:
        for i, v in enumerate(batch):
            batch[i] = to_precision(v, dtype, ignore_list)
    elif isinstance(batch, torch.Tensor):
        if batch.dtype.is_floating_point and batch.dtype != dtype:
            batch = batch.to(dtype, non_blocking=True)
    return batch


def iterate_nested_keys(nested_dict: Dict[str, Any]) -> Generator[Tuple[str, ...], None, None]:
    """
    Recursively iterates over all keys in a nested dictionary and yields the key paths as tuples.
    ...
    for key, value in nested_dict.items():
        if isinstance(value, dict):
            for sub_key_path in iterate_nested_keys(value):
                yield (key,) + sub_key_path
        else:
            yield (key,)


def get_value_from_nested_dict(nested_dict: Dict[str, Any],
                               key_path: Tuple[str, ...],
                               default: Any = None) -> Any:
    """
    Retrieves the value from a nested dictionary using a tuple of keys.

    If any key in the path is not present, returns the default value.
    """
    current = nested_dict
    for key in key_path:
...
def set_value_in_nested_dict(nested_dict: Dict[str, Any],
                             key_path: Tuple[str, ...],
                             value: Any) -> None:
    """
    Sets a value in a nested dictionary using a tuple of keys.

    Creates nested dictionaries along the path if they do not exist.
    """
    current = nested_dict
...
def average_dict(dict_list: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
    Computes the average value for each key (including nested keys) across a list of dictionaries,
    and returns a nested dictionary with the same key structure, where each key's value is the computed average.

    If a key is missing in all dictionaries or if all values are NaN, the result will be NaN.
    """
    # Collect all nested key paths from the dictionaries
    all_key_paths = set()
    for single_dict in dict_list:
        all_key_paths.update(iterate_nested_keys(single_dict))
    sorted_key_paths = sorted(all_key_paths)

    # Construct the resulting dictionary with computed averages
    averaged_dict: Dict[str, Any] = {}
    for key_path in sorted_key_paths:
        values = []
        for single_dict in dict_list:
            value = get_value_from_nested_dict(single_dict, key_path)
            if value is not None and not (isinstance(value, float) and math.isnan(value)):
                values.append(value)
        # Calculate the average for the current key path
        average_value = sum(values) / len(values) if values else float('nan')
        set_value_in_nested_dict(averaged_dict, key_path, average_value)
...
def load_depth(depth_file: str):
    if depth_file.endswith('.npy'):
        ...
        depth = cv2.imread(depth_file, cv2.IMREAD_UNCHANGED | cv2.IMREAD_ANYDEPTH).astype(np.float32) / 256.0
        depth = depth[..., None]
    else:
...
def save_image(img_path: str, img: np.ndarray, jpeg_quality=75, png_compression=9, save_dtype=np.uint8):
    ...
    if img.shape[0] < img.shape[-1] and (img.shape[0] == 3 or img.shape[0] == 4 or img.shape[0] == 1): img = np.transpose(img, (1, 2, 0))
    if np.issubdtype(img.dtype, np.integer):
...
def load_image_from_bytes(buffer: np.ndarray, ratio=1.0, normalize=False, decode_flag=cv2.IMREAD_UNCHANGED, source_path: str = ""):
    ...
    if image is None:
        # OpenCV occasionally returns None for valid JPEG buffers under heavy
        # eval I/O. Fall back to PIL so long runs do not die on a single decode.
        try:
            pil = Image.open(BytesIO(buffer.tobytes()))
            if decode_flag == cv2.IMREAD_GRAYSCALE:
                pil = pil.convert('L')
            else:
                if pil.mode not in ('RGB', 'RGBA', 'L', 'LA', 'I;16', 'I'):
                    pil = pil.convert('RGBA' if 'A' in pil.getbands() else 'RGB')
            image = np.array(pil)
            if image.ndim == 3 and image.shape[-1] >= 3 and decode_flag != cv2.IMREAD_GRAYSCALE:
                rgb = image[..., :3][..., ::-1]  # PIL is RGB, project expects OpenCV BGR
                if image.shape[-1] > 3:
                    image = np.concatenate([rgb, image[..., 3:]], axis=-1)
                else:
                    image = rgb
            log(f"cv2.imdecode returned None, fallback to PIL for {source_path or '<unknown>'}")
        except Exception as e:
            raise ValueError(f"Failed to decode image for {source_path or '<unknown>'}: {e}") from e
...
def load_image_bytes(im: str):
    if im.endswith('.exr'):
        os.environ["OPENCV_IO_ENABLE_OPENEXR"] = "1"
    # Use a while loop to handle potential OSError
    while True:
        try:
            ...
            log(f"Error loading image {im}, hostname {hostname()}, error: {e}, retrying in 1.0s")
            time.sleep(1.0)  # lower io frequency
    return buffer
...
def sparse_resize(sparse_depth, H, W):
    # Create an empty array for the resized depth map
    resized_depth = np.zeros((H, W), dtype=sparse_depth.dtype)

    # Find the non-zero indices in the sparse depth map
    non_zero_indices = np.argwhere(sparse_depth > 0)
    ...
    scale_x = W / sparse_depth.shape[1]
    scale_y = H / sparse_depth.shape[0]

    # Calculate the new coordinates
    new_x = (non_zero_indices[:, 1] * scale_x).astype(int)
    new_y = (non_zero_indices[:, 0] * scale_y).astype(int)

    # Make sure the new coordinates are within bounds
    new_x = np.clip(new_x, 0, W - 1)
    new_y = np.clip(new_y, 0, H - 1)

    # Convert the 2D coordinates to flat indices using np.ravel_multi_index
    flat_indices = np.ravel_multi_index((new_y, new_x), resized_depth.shape)
    ...
    min_values = np.full_like(unique_indices, max_value, dtype=sparse_depth.dtype)

    # Update the minimum values for each unique index
    for idx, value in enumerate(non_zero_values):
        target_idx = inverse_indices[idx]
        if value < min_values[target_idx]:
            min_values[target_idx] = value

    # Assign the minimum values to the resized depth map
    resized_depth.flat[unique_indices] = min_values
    return resized_depth
...
def load_resize_undist_im_bytes(imp: str,
                                ...
                                sparse_interpolate: bool = False,
                                use_png_depth: bool = False,
                                use_dynamic_masks: bool = False,
                                use_segs: bool = False,
                                depth_mask_th: float = 1.0
                                ):

    # Load image -> resize -> undistort -> save to bytes (jpeg)
    if use_png_depth:
        img = cv2.imread(imp, cv2.IMREAD_UNCHANGED | cv2.IMREAD_ANYDEPTH).astype(np.float32) / 256.0
    elif use_dynamic_masks or use_segs:
        img = cv2.imread(imp, cv2.IMREAD_UNCHANGED)
    else:
        if decode:
            img = load_image_from_bytes(load_image_bytes(imp), decode_flag=decode_flag, source_path=imp)[..., :3]  # cv2 decoding (fast)
    
    oH, oW = img.shape[:2]
    if sparse_interpolate and depth_mask_th < 1.0 and int(imp.split("/")[-2]) == 0:  #  hard code, camera front
        img[int(oH * depth_mask_th):, :] = 0.0
    ...
    if not (
        (
            (isinstance(ratio, float) and ratio == 1.0) or
            (isinstance(ratio, list) and ratio[0] == 1.0 and ratio[1] == 1.0)
        ) and
        (
            (isinstance(max_size, int) and max_size < 0) or
            (isinstance(max_size, list) and max_size[0] < 0 and max_size[1] < 0)
        )
    ):
        if isinstance(max_size, int) and max_size > 0:
            # Overwrite the ratio to fit the max size
            ratio = max_size / max(oH, oW)
        elif isinstance(max_size, list):
            if max_size[0] > 0 and max_size[1] > 0:
                ratio = min(max_size[0] / oH, max_size[1] / oW)
            elif max_size[0] > 0:
                ratio = max_size[0] / oH
            elif max_size[1] > 0:
                ratio = max_size[1] / oW
        ...
        if sparse_interpolate:
            img = sparse_resize(img, H, W)  # H, W, 3, uint8, only support depth for now
        else:
            img = cv2.resize(img, (W, H), interpolation=interpolation)  # H, W, 3, uint8
...
def load_resize_undist_ims_bytes(ims: np.ndarray,
                                 ...
                                 max_size: Union[int, List[int]] = -1,
                                 align_size: Union[int, List[int]] = 1,
                                 center_crop_size: List[int] = [-1, -1],
    ...
    # Ds = np.broadcast_to(Ds[:, None], (sh + (D, 1)))
    ...
    dist_dim = int(np.prod(Ds.shape[len(sh):]))
    Ds = Ds.reshape((np.prod(sh), dist_dim))
    ...
    if isinstance(max_size, list):
        max_size = np.asarray(max_size)  # avoid expansion

    if isinstance(align_size, list):
        align_size = np.asarray(align_size)  # avoid expansion
    ...
    out = parallel_execution(ims, Ks, Ds, ratio, max_size, align_size, center_crop_size,
