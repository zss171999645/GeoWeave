...
try:
    ...
    HAS_SDPA_KERNEL = hasattr(nn, "attention") and hasattr(nn.attention, "sdpa_kernel")
except ImportError:
    SDPBackend = None
...
def get_sdpa_kernel_context(use_flash: bool):
    if not HAS_SDPA_KERNEL or SDPBackend is None:
        return nullcontext()
    if use_flash:
        return nn.attention.sdpa_kernel(SDPBackend.FLASH_ATTENTION)
    return nn.attention.sdpa_kernel([SDPBackend.MATH, SDPBackend.EFFICIENT_ATTENTION])
...
class FlashAttention(Attention):
    def forward(self, x: Tensor, attn_bias=None) -> Tensor:
        ...
        with get_sdpa_kernel_context(q.dtype == torch.bfloat16):
