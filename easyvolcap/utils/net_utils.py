...
# from torch.distributed.checkpoint.state_dict import set_model_state_dict, set_optimizer_state_dict, get_model_state_dict, get_optimizer_state_dict, StateDictOptions
...
class VolumetricVideoModule(nn.Module):
    ...
    @property
    def network(self):
        network: Union["DepthModel"] = self.unregistered[0]
        return network
...
def load_pretrained(model_dir: str, resume: bool = True, epoch: int = -1, ext: str = '.npz', remove_if_not_resuming: bool = False, warn_if_not_exist: bool = False):
    if not resume:  # do not auto-remove trained weights on non-resume
        if remove_if_not_resuming:
            log(yellow(f"Skip removing trained weights (resume=False): {blue(model_dir)}"))
        return None, None
    ...
    if ext == '.pt' or ext == '.pth' or model_path.endswith('.pt') or model_path.endswith('.pth'):
        pretrained = torch.load(model_path, 'cpu', weights_only=False)
    else:
        from easyvolcap.utils.data_utils import to_tensor
        model = to_tensor(dict(**np.load(model_path)))
        epoch = model.pop('epoch', -1)
        pretrained = dict(model=model, epoch=epoch)  # the npz files do not contain training parameters
...
def load_controller(
    ...
    # Optimizer state need to be scattered to all ranks
    if optimizer is not None and 'optimizer' in pretrained.keys():
        total_optim_state = pretrained['optimizer'] if not get_rank() else {}
        # TODO: find out why the total_optim_state cannot be type of `dotdict`, otherwise the scatter will fail
        # Changing the default `dotdict` type return of `load_pretrained()` to `dict`
        shard_optim_state = FSDP.scatter_full_optim_state_dict(total_optim_state, model)
        optimizer.load_state_dict(shard_optim_state)
    ...
    epoch = pretrained['epoch']
    if isinstance(epoch, torch.Tensor) or isinstance(epoch, np.ndarray): epoch = epoch.item()
    log(f'Loaded controller {blue(model_path)} at epoch {blue(epoch)}')
    return epoch + 1
...
def save_npz_fsdp(model: nn.Module,
                  model_dir: str = '',
                  epoch: int = -1,
                  latest: int = True,
                  ):
    ...
    # Only rank 0 will save the state dict to the disk
    if not get_rank():
        param_dict = to_numpy(state_dict)  # a shallow dict (dotdict)
        param_dict.epoch = epoch  # just 1 scalar
        os.makedirs(dirname(npz_path), exist_ok=True)
...
def save_model(model: nn.Module,
    ...
    is_main_rank = (get_rank() == 0)

    if optimizer is not None and is_distributed:
        consolidate_state_dict = getattr(optimizer, "consolidate_state_dict", None)
        if callable(consolidate_state_dict):
            # ZeroRedundancyOptimizer only exposes full optimizer state on the target rank.
            try:
                kwargs = {}
                if "to" in inspect.signature(consolidate_state_dict).parameters:
                    kwargs["to"] = 0
                consolidate_state_dict(**kwargs)
    ...
    if len(pts) <= save_lim:
        return
    # Auto-removal disabled to avoid losing checkpoints.
    return


# https://www.youtube.com/watch?v=3HeGBJamfAs
def save_model_fsdp(model: nn.Module,
                    optimizer: Union[torch.optim.Optimizer, None] = None,
                    scheduler: Union[torch.optim.lr_scheduler._LRScheduler, None] = None,
                    moderator: Union[nn.Module, None] = None,
                    model_dir: str = '',
    ...
    def _summon_full_params_ctx(fsdp_model: FSDP, offload_to_cpu: bool = False):
        kwargs = {"writeback": False, "rank0_only": True}
        if offload_to_cpu and "offload_to_cpu" in inspect.signature(FSDP.summon_full_params).parameters:
            kwargs["offload_to_cpu"] = True
        return FSDP.summon_full_params(fsdp_model, **kwargs)

    model_state_dict = None
    if isinstance(model, FSDP):
        try:
            # Prefer summon_full_params to avoid FULL_STATE_DICT issues with some params.
            with _summon_full_params_ctx(model, offload_to_cpu=False):
                if not get_rank():
                    model_state_dict = model.module.state_dict()
                else:
                    model_state_dict = {}
        except Exception as e:
            log(yellow(f"FSDP summon_full_params failed, falling back to FULL_STATE_DICT: {e}"))
            with FSDP.state_dict_type(
                model,
                StateDictType.FULL_STATE_DICT,
                FullStateDictConfig(offload_to_cpu=True, rank0_only=True),
    ...
    if optimizer is not None:
        ckpt_dict['optimizer'] = FSDP.full_optim_state_dict(model, optimizer)

    if scheduler is not None:
        ckpt_dict['scheduler'] = scheduler.state_dict()

    if moderator is not None:
        ckpt_dict['moderator'] = moderator.state_dict()

    # Save to the disk only in rank 0
    if not get_rank():
        if not os.path.exists(model_dir):
            os.makedirs(model_dir, exist_ok=True)
...
# def fsdp2_model_shard(
#     model: nn.Module,
#     shard_class_list: List[str] = None,
#     shard_variable_list: List[str] = None,
#     mesh: DeviceMesh = None,
#     reshard_after_forward: bool = True,
# ):
#     # Return directly if shard_list is None
#     if shard_class_list is None and shard_variable_list is None:
#         return model

#     def recursive_fully_shard(module: nn.Module, module_path: str):
#         # Depth-first search to find all children modules
#         for name, child in module.named_children():
#             path = f"{module_path}.{name}" if module_path else name
#             recursive_fully_shard(child, path)

#         # Do the sharding
#         if module.__class__.__name__ in shard_class_list or module_path in shard_variable_list:
...
#     # Shard the model from fine-grain to coarse-grain
#     recursive_fully_shard(model, '')
...
#             path = f"{module_path}.{name}" if module_path else name
#             # Do the sharding
#             if child.__class__.__name__ in shard_class_list or path in shard_variable_list:
