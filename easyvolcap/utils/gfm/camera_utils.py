...
def check_has_same_row(
    ...
    return np.isclose(v0, v1).all(-1).any()
...
def load_igcs_pose_v1(
    ...
    for _, r in df[df['frameIndex'] >= 0].iterrows():
        ...
        df_cur_gidx = df[df['gameFrameIndex'] == gidx]
        df_cur_gidx = df_cur_gidx[df_cur_gidx['frameIndex'] < 0]

        # The first row is the camera parameters
        if len(df_cur_gidx) == 0:
            log(f'No camera parameters found for game index {gidx}')
            exit()
            # raise ValueError('Please check this')

        row = df_cur_gidx.iloc[0].copy()

        # Check if the camera parameters are the same
        if not check_all_same_row(
            df_cur_gidx[:-max_allowd_diff_pose_timestamps], row, check_keys=check_keys
        ):
            log(f'Camera parameters are not the same for game index {gidx}')
            exit()
            # raise ValueError('Please check this')

        df_nxt_gidx = df[df['gameFrameIndex'] == gidx - 1]
        df_nxt_gidx = df_nxt_gidx[df_nxt_gidx['frameIndex'] < 0]

        # Check if the camera parameters are the same
        if not check_all_same_row(
            df_nxt_gidx, row, check_keys=check_keys
        ):
            log(f'Camera parameters are not the same for game index {gidx - 1}')
            exit()
            # raise ValueError('Please check this')

        # Update the row's frame index
