    ...
    def read(self, key, dt="mat"):
        ...
        if dt == "mat":
            if isinstance(value, np.ndarray):
                ...
                rows = value['rows']
                cols = value['cols']
                value = np.array(value['data']).reshape(rows, cols)
            else:
...
class FileStoragePkl(FileStorageDict):
...
def read_camera(
    ...
    use_dict: bool = False,
    use_pkl: bool = False,
    pkl_intri_file: str = '__tmp_intri.pkl',
    pkl_extri_file: str = '__tmp_extri.pkl',
) -> dotdict:
    ...
    if use_pkl:
        if not exists(intri_path.replace(intri_path.split('/')[-1], pkl_intri_file)): use_pkl = False
        else: intri_path = intri_path.replace(intri_path.split('/')[-1], pkl_intri_file)
        if not exists(extri_path.replace(extri_path.split('/')[-1], pkl_extri_file)): use_pkl = False
        else: extri_path = extri_path.replace(extri_path.split('/')[-1], pkl_extri_file)

    # Select the storage type
    if use_pkl: storage = FileStoragePkl
    elif use_dict: storage = FileStorageDict
    else: storage = FileStorage

    # Load the camera parameters
    intri = storage(intri_path)
    extri = storage(extri_path)
    ...
    for cam in cam_names:
        ...
        if D is None: D = np.zeros((5, 1))  # default to 0 distortion
        cams[cam].D = D
...
def write_3ddr(cameras: dict, path: str, extri_name: str = ''):
    os.makedirs(path, exist_ok=True)
    if not extri_name:
        extri_name = join(path, 'extri_3ddr.yml')
    extri = FileStorage(extri_name, True)
    cam_names = [key_.split('.')[0] for key_ in cameras.keys()]
    extri.write('names', cam_names, 'list')
