...
def print_profile_stats_if_slow(name, time_threshold=10):
    import cProfile as cp

    def wrapper(func):
        def inner(*args, **kwargs):
            t0 = time.perf_counter()
            # profile the getitem time and sort by cumsum , print top 10
            pr = cp.Profile()
            pr.enable()
            ...
            if t1 - t0 > time_threshold:
                log(red(f"rank {get_rank()} {name} time: {t1 - t0}"))
