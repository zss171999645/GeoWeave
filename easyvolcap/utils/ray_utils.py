...
def get_plucker(ray_o, ray_d):
    """ Compute plucker embedding for rays

    Args:
        ray_o: torch.Tensor (..., 3), ray origins
        ray_d: torch.Tensor (..., 3), ray directions
    
    Returns:
        plucker: torch.Tensor (..., 6), plucker coordinates (ray_d, ray_o x ray_d)
    """
    # Normalize the ray direction
    ray_d = normalize(ray_d)  # (B, P, 3)
    plucker = torch.cat([ray_d, torch.cross(ray_o, ray_d, dim=-1)], dim=-1)
    return plucker
