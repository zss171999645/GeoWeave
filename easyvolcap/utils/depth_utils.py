...
class Depth(Enum):
    ...
    LINEAR = auto()  # the default source indexing
    INVERSE = auto()  # will index the source view in zigzag order


class Scale(Enum):
    # Type of scale factor calculation
    DEPTH = auto()  # use the maximum depth value
    XYZ = auto()  # use the maximum xyz coordinate value
    DIST = auto()  # thte the maximum distance value
    MANUAL = auto()  # use the manually specified scale value


def convert_linear_depth(dpt: torch.Tensor, scale: float = 1.0):
    dpt = dpt * scale
    return dpt


def convert_inverse_depth(dpt: torch.Tensor, near: float, far: float, scale: float = 1.0):
    # Convert the depth map to inverse depth map
    dpt = dpt * scale  # (..., 1)
    dpt = 1 / (dpt * (1 / near - 1 / far) + 1 / far)  # (..., 1)
    return dpt
...
def normalize(
    ...
    """ Normalize xyz by the average distance of the valid points.
    Args:
    ...
    Returns:
        xyz (torch.Tensor): (B, ..., 3)
        avg_scale (torch.Tensor): (B, ...)
    """
    # Compute the distance of each point to the origin
    dists = xyz.norm(dim=-1)  # (B, ...)

    # Compute the sum of the distances and the number of valid points
    D = (dists * msk).sum(dim=list(range(1, dists.ndim)))  # (B,)
    M = msk.sum(dim=list(range(1, dists.ndim)))  # (B,)

    # Compute the average distance of the valid points
    avg_scale = (D / (M + eps)).clamp(min=eps, max=1e3)  # (B,)
    xyz = xyz / avg_scale.view(-1, *([1] * (xyz.ndim - 1)))  # (B, ..., 3)

    return xyz, avg_scale
...
def normalize_depth(
    ...
    if mask is not None: depth[~mask] = 0  # fill the invalid depth values with 0
    n = int(depth.numel() * p)
...
def depth_curve_fn(
    ...
    if mask is not None and mask.shape[-1] == 1: mask = mask.squeeze(-1)
    depth = depth.clone()
    depth = normalize_depth(depth.float(), mask, p)  # add `.float()` to avoid mixed precision error
    if cm == 'spectral': depth = 1 - depth  # for better visualization
    depth = colormap(depth, cm)
    if mask is not None: depth[~mask] = 0
    return depth
...
def conf_curve_fn(
    ...
    n = int(conf.numel() * p)
    near = conf.ravel().topk(n, largest=False)[0].max()
    far = conf.ravel().topk(n, largest=True)[0].min()
    conf = (conf - near) / (far - near)
    conf = conf.clip(0, 1)
    conf = colormap(conf, cm)
    return conf
...
def depth_specific_quantile(dpt, quantile):
    """
    针对深度图的特定优化
    """
    dpt_valid = dpt.flatten()
    
    # 如果数据量仍然太大，进行采样
    if dpt_valid.numel() > 500000:
        n_samples = min(500000, dpt_valid.numel())
        indices = torch.randperm(dpt_valid.numel())[:n_samples]
        dpt_sampled = dpt_valid[indices]
        return torch.quantile(dpt_sampled, quantile)
    else:
        return torch.quantile(dpt_valid, quantile)