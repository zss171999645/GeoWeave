...
cm_cpu_store.spectral = matplotlib.colormaps['Spectral']
...
def colormap(v: torch.Tensor, cm: str = 'virdis'):
    ...
    *x*, *y0*, *y1* tuples.  In each sequence, *x* must increase
    ...
    if dv not in cm_gpu_store[cm]:
        ...
        elif isinstance(mapping, matplotlib.colors.Colormap):
            mapping = mapping
            function = colormap_matplotlib
        else:
            mapping = mapping
...
def colormap_matplotlib(v: torch.Tensor, cm: matplotlib.colors.Colormap, bytes: bool = False):
    device = v.device
    v = v[..., 0] if v.shape[-1] == 1 else v
    v = v.cpu()
    v = cm(v, bytes=bytes)[..., :3]
