...
from easyvolcap.official_vggt.utils.pose_enc import extri_intri_to_pose_encoding
from easyvolcap.official_vggt.train_utils.general import check_and_fix_inf_nan
    ...
    def forward(self, predictions, batch) -> torch.Tensor:
        ...
        if base_tensor is None:
            for value in predictions.values():
                ...
                if isinstance(value, (list, tuple)) and value and torch.is_tensor(value[0]):
                    base_tensor = value[0]
                    break
        total_loss = base_tensor.new_zeros(()) if base_tensor is not None else torch.zeros(())
        has_loss = False
        loss_dict = {}
        ...
        if not has_loss:
            if base_tensor is not None:
                total_loss = base_tensor.new_zeros((), requires_grad=True)
            else:
                total_loss = torch.zeros((), requires_grad=True)
        loss_dict["objective"] = total_loss

        return loss_dict


def _nan_safe_min_max(tensor: torch.Tensor):
    if tensor is None:
        return None, None
...
def _sanitize_gt(tensor: torch.Tensor):
    has_nan = torch.isnan(tensor).any()
    has_inf = torch.isinf(tensor).any()
    if bool(has_nan) or bool(has_inf):
        tensor = torch.nan_to_num(tensor, nan=0.0, posinf=0.0, neginf=0.0)
    return tensor, has_nan, has_inf
...
def compute_point_loss(predictions, batch, gamma=1.0, alpha=0.2, gradient_loss_fn = None, valid_range=-1, **kwargs):
    ...
    pred_points_min, pred_points_max = _nan_safe_min_max(pred_points)
    pred_points_conf_min, pred_points_conf_max = _nan_safe_min_max(pred_points_conf)
    gt_points = batch['world_points']
    gt_points_mask = batch['point_masks']

    gt_points, gt_points_has_nan, gt_points_has_inf = _sanitize_gt(gt_points)
    ...
    if gt_points_mask.sum() < 100:
        ...
        # Keep both prediction branches in graph so DDP bucket usage stays consistent across ranks.
        dummy_loss = (0.0 * pred_points).mean() + (0.0 * pred_points_conf).mean()
        loss_dict = {
            f"loss_conf_point": dummy_loss,
            f"loss_reg_point": dummy_loss,
            f"loss_grad_point": dummy_loss,
        }
        if pred_points_min is not None:
            loss_dict.update({
                "stat_pred_points_min": pred_points_min,
                "stat_pred_points_max": pred_points_max,
                "stat_pred_points_conf_min": pred_points_conf_min,
                "stat_pred_points_conf_max": pred_points_conf_max,
                "stat_gt_points_has_nan": gt_points_has_nan.float(),
                "stat_gt_points_has_inf": gt_points_has_inf.float(),
            })
        return loss_dict
...
def compute_depth_loss(predictions, batch, gamma=1.0, alpha=0.2, gradient_loss_fn = None, valid_range=-1, **kwargs):
    ...
    pred_depth_min, pred_depth_max = _nan_safe_min_max(pred_depth)
    pred_depth_conf_min, pred_depth_conf_max = _nan_safe_min_max(pred_depth_conf)

    gt_depth = batch['depths']
    gt_depth, gt_depth_has_nan, gt_depth_has_inf = _sanitize_gt(gt_depth)
    ...
    if gt_depth_mask.sum() < 100:
        # If there are less than 100 valid points, skip this batch.
        # Keep both prediction branches in graph so DDP bucket usage stays consistent across ranks.
        dummy_loss = (0.0 * pred_depth).mean() + (0.0 * pred_depth_conf).mean()
        loss_dict = {
            f"loss_conf_depth": dummy_loss,
            f"loss_reg_depth": dummy_loss,
            f"loss_grad_depth": dummy_loss,
        }
        if pred_depth_min is not None:
            loss_dict.update({
                "stat_pred_depth_min": pred_depth_min,
                "stat_pred_depth_max": pred_depth_max,
                "stat_pred_depth_conf_min": pred_depth_conf_min,
                "stat_pred_depth_conf_max": pred_depth_conf_max,
                "stat_gt_depth_has_nan": gt_depth_has_nan.float(),
                "stat_gt_depth_has_inf": gt_depth_has_inf.float(),
            })
        return loss_dict
