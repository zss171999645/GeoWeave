...
class GradientClipper:
    """
    Gradient clipping utils that works with DDP and supports different clipping
    ...
    def setup_clipping(self, model: nn.Module) -> None:
        ...
        if len(remaining_params) > 0:
            raise ValueError(f"{len(remaining_params)} parameters are not configured for gradient clipping")
