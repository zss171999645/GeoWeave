...
def configurable_entrypoint(SEPERATION='--', LAUNCHER='', EASYVOLCAP=EASYVOLCAP,
    ...
    cmd = ' '.join(args)
    log(f"Invoking: {cmd}", markup=False)
...
def acc_entrypoint():
    # Accelerate managed distributed training
    configurable_entrypoint(LAUNCHER='accelerate launch', EASYVOLCAP=f'{easyvolcap}/easyvolcap/scripts/main.py', extra_easyvolcap_args=['accelerated=True'])
