...
try:
    ...
    # Some environments ship a broken yapf install (e.g. corrupted grammar pickle).
    # Config formatting is optional, so we fall back to unformatted text.
    FormatCode = None
...
class Config:
    ...
    def pretty_text(self):
        ...
        if FormatCode is not None:
            try:
                text, _ = FormatCode(text, style_config=yapf_style)  # verify is invalid
            except Exception:  # noqa: BLE001
                # Keep training/evaluation startup robust when yapf fails at runtime.
                pass
...
class DictAction(Action):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        for kv in values:
            if '=' not in kv:
                warnings.warn(f'Skip invalid override item: {kv}')
                continue
